...
async def order_consumer():
    ...
    try:
        async for signal_data in eventbus.subscribe_with_group(
            ...
            # Extract EventBus metadata BEFORE removing it
            message_id = signal_data.pop("_message_id", None)
            stream_name = signal_data.pop("_stream_name", None)
            group = signal_data.pop("_group_name", None)
            ...
            try:
                ...
                # SUCCESS: ACK the message only after successful execution
                if message_id and stream_name and group:
                    await eventbus.ack_message(stream_name, group, message_id)
                    logger.info(f"✅ Order intent processed and ACKed: {intent.symbol}")
                    
            except Exception as e:
                logger.error(f"❌ Order execution failed: {e}", exc_info=True)
                # FAILURE: ACK anyway to avoid infinite replay loop
                # Log terminal FAILED state for visibility
                try:
                    symbol = signal_data.get("symbol", "UNKNOWN")
                    side = signal_data.get("action", "UNKNOWN")
                    logger.info(f"🚫 TERMINAL STATE: FAILED | {symbol} {side} | Reason: {e}")
                except:
                    pass
                
                if message_id and stream_name and group:
                    await eventbus.ack_message(stream_name, group, message_id)
                    logger.warning(f"⚠️  Order intent ACKed despite failure (to avoid infinite loop): {message_id}")
