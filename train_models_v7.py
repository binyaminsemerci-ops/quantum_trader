#!/usr/bin/env python3
"""
NHiTS + PatchTST v7 Training - Saves models in DIRECT format (not checkpoint)
Compatible with unified_agents.py BaseAgent
"""
...
print("=" * 70)
print("NHiTS + PatchTST v7 Training (Direct Model Save)")
...
def calculate_features(df):
    ...
    for symbol in df["symbol"].unique():
        ...
        sdf["label"] = 1  # HOLD
        sdf.loc[future_return > 0.01, "label"] = 2  # BUY
        sdf.loc[future_return < -0.01, "label"] = 0  # SELL
...
if __name__ == "__main__":
    ...
    # SAVE AS WRAPPED MODEL (.pkl format for compatibility)
    nhits_wrapper = PyTorchClassifierWrapper(nhits, device='cpu')
    nhits_path = SAVE_DIR / f"nhits_v{timestamp}_v7.pkl"
    joblib.dump(nhits_wrapper, nhits_path)
    print(f"\n💾 NHiTS saved to {nhits_path}")
    
    # Save scaler
    scaler_path = SAVE_DIR / f"nhits_v{timestamp}_v7_scaler.pkl"
    joblib.dump(scaler, scaler_path)
    
    # Save metadata
    meta_path = SAVE_DIR / f"nhits_v{timestamp}_v7_meta.json"
    with open(meta_path, 'w') as f:
        json.dump({
            "features": FEATURES_V5,
            "version": f"nhits_v7_{timestamp}",
            "accuracy": nhits_acc,
            "type": "pytorch_classifier"
        }, f)
    
    # 5. Train and save PatchTST
    print("\n" + "=" * 70)
    ...
    # SAVE AS WRAPPED MODEL (.pkl format)
    patchtst_wrapper = PyTorchClassifierWrapper(patchtst, device='cpu')
    patchtst_path = SAVE_DIR / f"patchtst_v{timestamp}_v7.pkl"
    joblib.dump(patchtst_wrapper, patchtst_path)
    print(f"\n💾 PatchTST saved to {patchtst_path}")
    
    # Save scaler
    scaler_path = SAVE_DIR / f"patchtst_v{timestamp}_v7_scaler.pkl"
    joblib.dump(scaler, scaler_path)
    
    # Save metadata
    meta_path = SAVE_DIR / f"patchtst_v{timestamp}_v7_meta.json"
    with open(meta_path, 'w') as f:
        json.dump({
            "features": FEATURES_V5,
            "version": f"patchtst_v7_{timestamp}",
            "accuracy": patchtst_acc,
            "type": "pytorch_classifier"
        }, f)
    ...
    print("   Format: .pkl with sklearn-like .predict() interface")
    print("=" * 70)
