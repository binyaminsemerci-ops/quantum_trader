...
def check_system_status():
    ...
    try:
        ...
        status["frontend"]["node_version"] = node_result.stdout.strip() if node_result.returncode == 0 else None
        status["frontend"]["npm_version"] = npm_result.stdout.strip() if npm_result.returncode == 0 else None
...
def print_status_report(status):
    ...
    if frontend.get("node_available"):
        ...
        print(f"   ❌ Node.js: Ikke tilgjengelig")
    
    if frontend.get("npm_available"):
        print(f"   ✅ npm: {frontend.get('npm_version', 'ukjent versjon')}")
    else:
        print(f"   ❌ npm: Ikke tilgjengelig")
    
    print(f"   Dependencies: {'✅ Installert' if frontend.get('dependencies_installed') else '❌ Mangler'}")
    print(f"   Kan bygge: {'✅ Ja' if frontend.get('can_build') else '❌ Nei'}")
    
    # Tjenester
    services = status["services"]
    print(f"\n🌐 TJENESTER:")
    print(f"   Backend (port 8000): {'✅ Kjører' if services['backend_running'] else '❌ Ikke kjører'}")
    print(f"   Frontend (port 5173): {'✅ Kjører' if services['frontend_running'] else '❌ Ikke kjører'}")
    ...
    frontend_score = 0
    if frontend.get("node_available"): frontend_score += 0.3
    if frontend.get("npm_available"): frontend_score += 0.3 
    if frontend.get("dependencies_installed"): frontend_score += 0.2
    if frontend.get("can_build"): frontend_score += 0.2
    
    services_running = len([v for v in services.values() if v])
    
    print(f"\n📈 SAMLET VURDERING:")
    print(f"   Komponenter: {component_score:.1%} ({total_components}/{total_possible})")
    ...
    if overall_score >= 0.8:
        ...
        print(f"   💡 System er klar for utvikling og testing!")
    elif overall_score >= 0.6:
        print(f"   🟡 Status: BRA ({overall_score:.1%})")
        print(f"   💡 System er i hovedsak funksjonelt")
    else:
        print(f"   🔴 Status: TRENGER ARBEID ({overall_score:.1%})")
        print(f"   💡 Flere komponenter må fikses")
