#!/usr/bin/env python3
"""
Real-time signal monitor - viser AI confidence levels og venter på >=0.70 signals
"""
import asyncio
import os
...
async def monitor_signals():
    ...
    print(f"\n{'='*80}")
    print(f"[SEARCH] SIGNAL MONITOR - Venter på >=0.70 confidence signals")
    print(f"{'='*80}\n")
    ...
    while True:
        ...
        if high_confidence_signals:
            print(f"[TARGET] HIGH QUALITY SIGNALS: {len(high_confidence_signals)} (>=0.70)", end="")
            for action, conf in high_confidence_signals[:3]:
                print(f" | {action} {conf:.2f}", end="")
        elif medium_confidence_signals:
            print(f"[WARNING]  Medium signals: {len(medium_confidence_signals)} (0.65-0.69)", end="")
            top_3 = sorted(medium_confidence_signals, key=lambda x: x[1], reverse=True)[:3]
            for action, conf in top_3:
                print(f" | {action} {conf:.2f}", end="")
        else:
            print(f"⏳ Ingen signaler >=0.65 (venter...)", end="")
        
        if signal_count > 0:
            print(f" | [OK] {signal_count} EXECUTED", end="")
