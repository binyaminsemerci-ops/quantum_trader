# QUANTUM TRADER - DETALJERT ARKITEKTURDOKUMENTASJON
**Dato:** 17. desember 2025  
**Versjon:** 3.5 (Architecture v2)  
**Type:** Teknisk arkitektur og designdokumentasjon

---

## INNHOLDSFORTEGNELSE

1. [Arkitekturmønstre](#1-arkitekturmønstre)
2. [Komponentdiagram](#2-komponentdiagram)
3. [Sekvensdiagram](#3-sekvensdiagram)
4. [Data Arkitektur](#4-data-arkitektur)
5. [AI/ML Arkitektur](#5-aiml-arkitektur)
6. [Sikkerhet og Compliance](#6-sikkerhet-og-compliance)
7. [Skalerbarhet og Performance](#7-skalerbarhet-og-performance)

---

## 1. ARKITEKTURMØNSTRE

### 1.1 Event-Driven Architecture (EDA)

**Pattern:** Publish-Subscribe via Redis Streams

```
┌─────────────────────────────────────────────────────────────┐
│                  EVENT-DRIVEN ARCHITECTURE                  │
└─────────────────────────────────────────────────────────────┘

┌──────────────┐                              ┌──────────────┐
│  PRODUCERS   │                              │  CONSUMERS   │
└──────────────┘                              └──────────────┘
       │                                              ▲
       │  1. Publish event                            │
       ├────────────────────┐                         │
       │                    │                         │
       ▼                    ▼                         │
┌──────────┐          ┌──────────┐                   │
│ Market   │          │ Signal   │                   │
│ Scanner  │          │Generator │                   │
└──────────┘          └──────────┘                   │
       │                    │                         │
       └────────┬───────────┘                         │
                │                                     │
                │  2. Event to Redis Streams          │
                │                                     │
                ▼                                     │
        ┌────────────────┐                           │
        │  REDIS STREAMS │                           │
        │                │                           │
        │  • market.*    │                           │
        │  • signal.*    │                           │
        │  • trade.*     │                           │
        │  • rl.*        │                           │
        │  • risk.*      │                           │
        └────────────────┘                           │
                │                                     │
                │  3. Subscribe to events             │
                │                                     │
                └─────────────────────────────────────┘
                                                      │
                    ┌─────────────────────────────────┤
                    │                                 │
                    ▼                                 ▼
            ┌──────────────┐                  ┌──────────────┐
            │  Risk Guard  │                  │  Executor    │
            │  (Consumer)  │                  │  (Consumer)  │
            └──────────────┘                  └──────────────┘
```

**Fordeler:**
- ✅ Loose coupling mellom komponenter
- ✅ Horizontal skalerbarhet
- ✅ Fault tolerance (guaranteed delivery)
- ✅ Event replay capability

**Event Schema:**
```json
{
  "event_id": "uuid",
  "event_type": "signal.generated",
  "timestamp": "2025-12-17T10:30:00.123Z",
  "trace_id": "abc123",
  "source": "ai_engine",
  "data": {
    "symbol": "BTCUSDT",
    "direction": "LONG",
    "confidence": 0.82,
    "model": "ensemble"
  }
}
```

### 1.2 Domain-Driven Design (DDD)

**Bounded Contexts:**

```
┌─────────────────────────────────────────────────────────────┐
│                    QUANTUM TRADER SYSTEM                    │
└─────────────────────────────────────────────────────────────┘
                              │
        ┌─────────────────────┼─────────────────────┐
        │                     │                     │
        ▼                     ▼                     ▼
┌────────────────┐    ┌────────────────┐    ┌────────────────┐
│    TRADING     │    │   AI/ML        │    │    RISK        │
│    DOMAIN      │    │   DOMAIN       │    │   DOMAIN       │
│                │    │                │    │                │
│  • Orders      │    │  • Models      │    │  • Limits      │
│  • Positions   │    │  • Training    │    │  • Drawdown    │
│  • Execution   │    │  • Predictions │    │  • Emergency   │
└────────────────┘    └────────────────┘    └────────────────┘
        │                     │                     │
        └─────────────────────┴─────────────────────┘
                              │
                    ┌─────────┴─────────┐
                    │                   │
                    ▼                   ▼
            ┌────────────────┐  ┌────────────────┐
            │  PORTFOLIO     │  │  GOVERNANCE    │
            │  DOMAIN        │  │  DOMAIN        │
            │                │  │                │
            │  • Balancing   │  │  • Compliance  │
            │  • Analytics   │  │  • Audit       │
            └────────────────┘  └────────────────┘
```

**Domain Entities:**

```
TRADING DOMAIN:
├── Order (Value Object)
│   ├── id: OrderId
│   ├── symbol: Symbol
│   ├── side: OrderSide (LONG/SHORT)
│   ├── type: OrderType (MARKET/LIMIT)
│   ├── quantity: Decimal
│   └── price: Optional[Decimal]
│
├── Position (Aggregate Root)
│   ├── id: PositionId
│   ├── symbol: Symbol
│   ├── side: PositionSide
│   ├── size: Decimal
│   ├── entry_price: Decimal
│   ├── tp_price: Optional[Decimal]
│   ├── sl_price: Optional[Decimal]
│   └── pnl: Decimal
│
└── Trade (Entity)
    ├── id: TradeId
    ├── position: Position
    ├── orders: List[Order]
    ├── status: TradeStatus
    └── metadata: Dict

AI/ML DOMAIN:
├── Model (Aggregate Root)
│   ├── id: ModelId
│   ├── type: ModelType (XGB/LGBM/NHITS/PATCHTST)
│   ├── version: str
│   ├── metrics: ModelMetrics
│   └── artifact_path: Path
│
├── Prediction (Value Object)
│   ├── symbol: Symbol
│   ├── direction: Direction
│   ├── confidence: float
│   ├── model: ModelId
│   └── timestamp: datetime
│
└── Signal (Entity)
    ├── id: SignalId
    ├── predictions: List[Prediction]
    ├── consensus: ConsensusResult
    └── metadata: SignalMetadata
```

### 1.3 Repository Pattern

**Data Access Layer:**

```
┌─────────────────────────────────────────────────────────┐
│                    APPLICATION LAYER                    │
└─────────────────────────────────────────────────────────┘
                         │
                         │  Uses
                         ▼
┌─────────────────────────────────────────────────────────┐
│                  REPOSITORY INTERFACES                  │
│                                                         │
│  • TradeRepository                                      │
│  • PositionRepository                                   │
│  • ModelRepository                                      │
│  • SignalRepository                                     │
└─────────────────────────────────────────────────────────┘
                         │
                         │  Implements
                         ▼
┌─────────────────────────────────────────────────────────┐
│              CONCRETE IMPLEMENTATIONS                   │
│                                                         │
│  • PostgresTradeRepository                              │
│  • RedisPositionRepository                              │
│  • FileSystemModelRepository                            │
└─────────────────────────────────────────────────────────┘
                         │
                         │  Access
                         ▼
┌─────────────────────────────────────────────────────────┐
│                   DATA STORAGE                          │
│                                                         │
│  • PostgreSQL  • Redis  • File System                   │
└─────────────────────────────────────────────────────────┘
```

**Example Implementation:**

```python
# Repository Interface
class PositionRepository(ABC):
    @abstractmethod
    async def get(self, position_id: PositionId) -> Optional[Position]:
        pass
    
    @abstractmethod
    async def save(self, position: Position) -> None:
        pass
    
    @abstractmethod
    async def get_all_open(self) -> List[Position]:
        pass

# Concrete Implementation
class RedisPositionRepository(PositionRepository):
    def __init__(self, redis_client: Redis):
        self.redis = redis_client
    
    async def get(self, position_id: PositionId) -> Optional[Position]:
        data = await self.redis.get(f"position:{position_id}")
        return Position.from_dict(json.loads(data)) if data else None
    
    async def save(self, position: Position) -> None:
        await self.redis.set(
            f"position:{position.id}",
            json.dumps(position.to_dict())
        )
```

### 1.4 CQRS Pattern (Planned)

**Command Query Responsibility Segregation:**

```
┌─────────────────────────────────────────────────────────┐
│                    CLIENT REQUEST                       │
└─────────────────────────────────────────────────────────┘
                         │
            ┌────────────┴────────────┐
            │                         │
            ▼                         ▼
    ┌──────────────┐          ┌──────────────┐
    │   COMMANDS   │          │   QUERIES    │
    │   (Write)    │          │   (Read)     │
    └──────────────┘          └──────────────┘
            │                         │
            ▼                         ▼
    ┌──────────────┐          ┌──────────────┐
    │ Write Model  │          │  Read Model  │
    │              │          │              │
    │ • Validation │          │ • Denorm.    │
    │ • Business   │          │ • Optimized  │
    │   Logic      │          │ • Cached     │
    └──────────────┘          └──────────────┘
            │                         │
            ▼                         ▼
    ┌──────────────┐          ┌──────────────┐
    │ PostgreSQL   │          │  Redis       │
    │ (Master)     │          │  (Cache)     │
    └──────────────┘          └──────────────┘
```

**Status:** 🟡 Planlagt for fremtidig implementasjon

---

## 2. KOMPONENTDIAGRAM

### 2.1 Backend Komponenter

```
┌────────────────────────────────────────────────────────────────┐
│                      BACKEND SERVICE                           │
└────────────────────────────────────────────────────────────────┘
                              │
        ┌─────────────────────┼─────────────────────┐
        │                     │                     │
        ▼                     ▼                     ▼
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│   API Layer  │      │  Core Layer  │      │Domain Layer  │
│              │      │              │      │              │
│ • Routes     │◄─────│ • EventBus   │◄─────│ • Trading    │
│ • WebSocket  │      │ • Policy     │      │ • AI/ML      │
│ • Middleware │      │ • Logger     │      │ • Risk       │
└──────────────┘      └──────────────┘      └──────────────┘
        │                     │                     │
        └─────────────────────┴─────────────────────┘
                              │
        ┌─────────────────────┼─────────────────────┐
        │                     │                     │
        ▼                     ▼                     ▼
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│ Services     │      │ Repositories │      │  Utils       │
│              │      │              │      │              │
│ • Executor   │      │ • Trade      │      │ • Binance    │
│ • Risk       │      │ • Position   │      │ • Cache      │
│ • Portfolio  │      │ • Model      │      │ • Telemetry  │
└──────────────┘      └──────────────┘      └──────────────┘
```

### 2.2 AI Engine Komponenter

```
┌────────────────────────────────────────────────────────────────┐
│                       AI ENGINE                                │
└────────────────────────────────────────────────────────────────┘
                              │
        ┌─────────────────────┼─────────────────────┐
        │                     │                     │
        ▼                     ▼                     ▼
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│  Ensemble    │      │   Agents     │      │  Training    │
│  Manager     │      │              │      │              │
│              │      │ • XGBoost    │      │ • CLM        │
│ • Voting     │◄─────│ • LightGBM   │◄─────│ • Retraining │
│ • Consensus  │      │ • N-HiTS     │      │ • Evaluation │
│              │      │ • PatchTST   │      │              │
└──────────────┘      └──────────────┘      └──────────────┘
        │                     │                     │
        └─────────────────────┴─────────────────────┘
                              │
        ┌─────────────────────┼─────────────────────┐
        │                     │                     │
        ▼                     ▼                     ▼
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│  Feature     │      │   Models     │      │   RL v3      │
│  Engineer    │      │              │      │              │
│              │      │ • XGB pkl    │      │ • PPO        │
│ • 49 feat.   │      │ • LGBM pkl   │      │ • Training   │
│ • Unified    │      │ • NHITS pth  │      │ • Daemon     │
└──────────────┘      └──────────────┘      └──────────────┘
```

### 2.3 Hedge Fund OS Komponenter

```
┌────────────────────────────────────────────────────────────────┐
│                    HEDGE FUND OS v2                            │
└────────────────────────────────────────────────────────────────┘
                              │
        ┌─────────────────────┼─────────────────────┐
        │                     │                     │
        ▼                     ▼                     ▼
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│  Executive   │      │  Operations  │      │  Governance  │
│              │      │              │      │              │
│ • AI CEO     │      │ • AI CIO     │      │ • Compliance │
│ • AI CRO     │      │ • Portfolio  │      │ • Audit OS   │
│              │      │ • Risk Mgmt  │      │ • Regulation │
└──────────────┘      └──────────────┘      └──────────────┘
        │                     │                     │
        └─────────────────────┴─────────────────────┘
                              │
        ┌─────────────────────┼─────────────────────┐
        │                     │                     │
        ▼                     ▼                     ▼
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│ Coordination │      │  Decision    │      │  Federation  │
│              │      │  Layer       │      │              │
│ • Policy     │      │              │      │ • Multi-     │
│   Sync       │      │ • Explainable│      │   Agent      │
│              │      │   AI         │      │ • Consensus  │
└──────────────┘      └──────────────┘      └──────────────┘
```

---

## 3. SEKVENSDIAGRAM

### 3.1 Trade Execution Flow

```
┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
│ Executor │ │AI Engine │ │Risk Guard│ │ Binance  │ │ EventBus │
└─────┬────┘ └─────┬────┘ └─────┬────┘ └─────┬────┘ └─────┬────┘
      │            │            │            │            │
      │ 1. Request │            │            │            │
      │  Signal    │            │            │            │
      ├───────────►│            │            │            │
      │            │            │            │            │
      │            │ 2. Ensemble│            │            │
      │            │  Prediction│            │            │
      │            ├──────┐     │            │            │
      │            │      │     │            │            │
      │            │◄─────┘     │            │            │
      │            │            │            │            │
      │  3. Signal │            │            │            │
      │◄───────────┤            │            │            │
      │            │            │            │            │
      │            │            │4. Validate │            │
      │            │            │   Risk     │            │
      ├────────────┼───────────►│            │            │
      │            │            │            │            │
      │            │            │5. Approved │            │
      │◄───────────┼────────────┤            │            │
      │            │            │            │            │
      │            │            │            │6. Place    │
      │            │            │            │   Order    │
      ├────────────┼────────────┼───────────►│            │
      │            │            │            │            │
      │            │            │            │7. Filled   │
      │◄───────────┼────────────┼────────────┤            │
      │            │            │            │            │
      │            │            │            │            │8. Emit
      │            │            │            │            │ trade.*
      ├────────────┼────────────┼────────────┼───────────►│
      │            │            │            │            │
      │            │            │            │            │9. TP/SL
      │            │            │            │  Orders    │
      ├────────────┼────────────┼───────────►│            │
      │            │            │            │            │
```

### 3.2 Continuous Learning Flow

```
┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
│ Scheduler│ │   CLM    │ │Data Client│ │ Trainer  │ │Evaluator │
└─────┬────┘ └─────┬────┘ └─────┬────┘ └─────┬────┘ └─────┬────┘
      │            │            │            │            │
      │1. Trigger  │            │            │            │
      │  Retrain   │            │            │            │
      ├───────────►│            │            │            │
      │            │            │            │            │
      │            │2. Fetch    │            │            │
      │            │  Data      │            │            │
      │            ├───────────►│            │            │
      │            │            │            │            │
      │            │  3. 90d    │            │            │
      │            │    Data    │            │            │
      │            │◄───────────┤            │            │
      │            │            │            │            │
      │            │            │4. Train    │            │
      │            │            │  XGBoost   │            │
      │            ├────────────┼───────────►│            │
      │            │            │            │            │
      │            │            │5. Model    │            │
      │            │◄───────────┼────────────┤            │
      │            │            │            │            │
      │            │            │            │6. Evaluate │
      │            ├────────────┼────────────┼───────────►│
      │            │            │            │            │
      │            │            │            │7. Metrics  │
      │            │◄───────────┼────────────┼────────────┤
      │            │            │            │            │
      │            │8. Save     │            │            │
      │            │  Model     │            │            │
      │            ├──────┐     │            │            │
      │            │      │     │            │            │
      │            │◄─────┘     │            │            │
      │            │            │            │            │
```

### 3.3 Emergency Stop Flow

```
┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
│   ESS    │ │Risk Guard│ │ Executor │ │ Binance  │ │ Monitor  │
└─────┬────┘ └─────┬────┘ └─────┬────┘ └─────┬────┘ └─────┬────┘
      │            │            │            │            │
      │1. Detect   │            │            │            │
      │  Drawdown  │            │            │            │
      │  > 15%     │            │            │            │
      ├──────┐     │            │            │            │
      │      │     │            │            │            │
      │◄─────┘     │            │            │            │
      │            │            │            │            │
      │2. Trigger  │            │            │            │
      │  Emergency │            │            │            │
      ├───────────►│            │            │            │
      │            │            │            │            │
      │            │3. Stop     │            │            │
      │            │  New Trades│            │            │
      │            ├───────────►│            │            │
      │            │            │            │            │
      │            │            │4. Close    │            │
      │            │            │  Losing    │            │
      │            ├────────────┼───────────►│            │
      │            │            │            │            │
      │            │            │            │5. Filled   │
      │            │◄───────────┼────────────┤            │
      │            │            │            │            │
      │            │            │            │            │6. Alert
      │            │            │            │            │ Ops
      ├────────────┼────────────┼────────────┼───────────►│
      │            │            │            │            │
```

---

## 4. DATA ARKITEKTUR

### 4.1 Database Schema

**PostgreSQL Tables:**

```sql
-- Trades table
CREATE TABLE trades (
    id UUID PRIMARY KEY,
    symbol VARCHAR(20) NOT NULL,
    side VARCHAR(10) NOT NULL,  -- LONG/SHORT
    entry_price DECIMAL(20, 8) NOT NULL,
    quantity DECIMAL(20, 8) NOT NULL,
    leverage INTEGER,
    tp_price DECIMAL(20, 8),
    sl_price DECIMAL(20, 8),
    status VARCHAR(20) NOT NULL,  -- OPEN/CLOSED/CANCELLED
    pnl DECIMAL(20, 8),
    roi DECIMAL(10, 4),
    created_at TIMESTAMP NOT NULL,
    closed_at TIMESTAMP,
    metadata JSONB
);

-- Positions table
CREATE TABLE positions (
    id UUID PRIMARY KEY,
    trade_id UUID REFERENCES trades(id),
    symbol VARCHAR(20) NOT NULL,
    side VARCHAR(10) NOT NULL,
    size DECIMAL(20, 8) NOT NULL,
    entry_price DECIMAL(20, 8) NOT NULL,
    current_price DECIMAL(20, 8),
    unrealized_pnl DECIMAL(20, 8),
    margin DECIMAL(20, 8),
    leverage INTEGER,
    tp_price DECIMAL(20, 8),
    sl_price DECIMAL(20, 8),
    created_at TIMESTAMP NOT NULL,
    updated_at TIMESTAMP
);

-- Signals table
CREATE TABLE signals (
    id UUID PRIMARY KEY,
    symbol VARCHAR(20) NOT NULL,
    direction VARCHAR(10) NOT NULL,  -- LONG/SHORT
    confidence DECIMAL(5, 4) NOT NULL,
    price DECIMAL(20, 8) NOT NULL,
    model VARCHAR(50) NOT NULL,
    features JSONB,
    created_at TIMESTAMP NOT NULL,
    INDEX idx_symbol (symbol),
    INDEX idx_created (created_at)
);

-- Models table
CREATE TABLE models (
    id UUID PRIMARY KEY,
    type VARCHAR(50) NOT NULL,  -- xgb/lgbm/nhits/patchtst
    version VARCHAR(50) NOT NULL,
    artifact_path VARCHAR(255) NOT NULL,
    metrics JSONB NOT NULL,
    status VARCHAR(20) NOT NULL,  -- ACTIVE/SHADOW/RETIRED
    created_at TIMESTAMP NOT NULL,
    promoted_at TIMESTAMP
);

-- Retraining jobs table
CREATE TABLE retraining_jobs (
    job_id VARCHAR(100) PRIMARY KEY,
    type VARCHAR(20) NOT NULL,  -- FULL/INCREMENTAL
    status VARCHAR(20) NOT NULL,  -- PENDING/RUNNING/SUCCESS/FAILED
    models JSONB NOT NULL,
    started_at TIMESTAMP,
    completed_at TIMESTAMP,
    metrics JSONB,
    error_message TEXT
);
```

### 4.2 Redis Data Structures

**Key Patterns:**

```
# Policy Store
policy:universe         → Hash (universe config)
policy:risk            → Hash (risk config)
policy:execution       → Hash (execution config)

# Event Streams
events:market          → Stream (market events)
events:signal          → Stream (signal events)
events:trade           → Stream (trade events)
events:rl              → Stream (RL events)

# Cache
cache:prices:{symbol}  → Hash (price data, TTL: 60s)
cache:positions        → Hash (open positions, TTL: 30s)
cache:signals          → List (recent signals, TTL: 300s)

# Health
health:redis           → String (status)
health:binance         → String (status)
health:system          → Hash (metrics)

# Session/Auth
session:{token}        → Hash (user session, TTL: 3600s)
```

### 4.3 File System Structure

```
/app/
├── models/                      # ML Model artifacts
│   ├── xgboost_v20251213.pkl   # XGBoost model (332KB)
│   ├── lightgbm_v20251213.pkl  # LightGBM model (289KB)
│   ├── nhits_v20251213.pth     # N-HiTS model (22MB)
│   ├── patchtst_v20251213.pth  # PatchTST model (2.8MB)
│   └── metadata.json            # Model metadata
│
├── data/                        # Application data
│   ├── price_history/          # Historical price data
│   │   ├── BTCUSDT_1m.json
│   │   ├── ETHUSDT_1m.json
│   │   └── ...
│   ├── rl_v3/                  # RL model checkpoints
│   │   ├── ppo_model.pt
│   │   └── training_log.json
│   └── backups/                # Database backups
│
└── logs/                       # Application logs
    ├── backend.log             # Main backend log
    ├── ai_engine.log           # AI engine log
    ├── trading.log             # Trading execution log
    └── error.log               # Error log
```

---

## 5. AI/ML ARKITEKTUR

### 5.1 Model Training Pipeline

```
┌───────────────────────────────────────────────────────────────┐
│                  ML TRAINING PIPELINE                         │
└───────────────────────────────────────────────────────────────┘

┌──────────────┐
│ 1. Data      │
│  Collection  │
│              │
│ • 100 symbols│
│ • 90 days    │
│ • 5m candles │
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ 2. Feature   │
│  Engineering │
│              │
│ • 49 features│
│ • Normalize  │
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ 3. Train     │
│  Models      │
│              │
│ • XGBoost    │ ─┐
│ • LightGBM   │  │
│ • N-HiTS     │  │ Parallel
│ • PatchTST   │  │ Training
└──────┬───────┘ ─┘
       │
       ▼
┌──────────────┐
│ 4. Evaluate  │
│  Performance │
│              │
│ • RMSE       │
│ • MAE        │
│ • R²         │
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ 5. Shadow    │
│  Testing     │
│              │
│ • 100+ preds │
│ • Compare    │
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ 6. Promote   │
│  to ACTIVE   │
│              │
│ • Replace    │
│ • Backup old │
└──────────────┘
```

### 5.2 Ensemble Prediction Flow

```
┌───────────────────────────────────────────────────────────────┐
│              ENSEMBLE PREDICTION WORKFLOW                     │
└───────────────────────────────────────────────────────────────┘

    Input: Market Data (OHLCV + Features)
             │
             ▼
    ┌────────────────┐
    │ Feature        │
    │ Engineering    │
    │ (49 features)  │
    └────────┬───────┘
             │
             ├──────────────────┬──────────────────┬──────────────┐
             │                  │                  │              │
             ▼                  ▼                  ▼              ▼
    ┌────────────┐     ┌────────────┐     ┌────────────┐ ┌────────────┐
    │  XGBoost   │     │ LightGBM   │     │   N-HiTS   │ │ PatchTST   │
    │            │     │            │     │            │ │            │
    │ Pred: 0.75 │     │ Pred: 0.82 │     │ Pred: 0.88 │ │ Pred: 0.70 │
    │ Dir: LONG  │     │ Dir: LONG  │     │ Dir: LONG  │ │ Dir: LONG  │
    └────────┬───┘     └────────┬───┘     └────────┬───┘ └────────┬───┘
             │                  │                  │              │
             └──────────────────┴──────────────────┴──────────────┘
                                 │
                                 ▼
                        ┌────────────────┐
                        │  Weighted      │
                        │  Voting        │
                        │                │
                        │ 0.75 * 0.25 =  │
                        │   0.1875       │
                        │ 0.82 * 0.25 =  │
                        │   0.2050       │
                        │ 0.88 * 0.30 =  │
                        │   0.2640       │
                        │ 0.70 * 0.20 =  │
                        │   0.1400       │
                        │ ──────────────│
                        │ Total: 0.7965  │
                        └────────┬───────┘
                                 │
                                 ▼
                        ┌────────────────┐
                        │  Consensus     │
                        │  Check         │
                        │                │
                        │ 4/4 agree LONG │
                        │ → High Conf    │
                        └────────┬───────┘
                                 │
                                 ▼
                         Final Signal:
                         LONG @ 0.80 confidence
```

### 5.3 Reinforcement Learning Architecture

```
┌───────────────────────────────────────────────────────────────┐
│                 RL V3 PPO ARCHITECTURE                        │
└───────────────────────────────────────────────────────────────┘

    Environment (Market)
             │
             │ Observation
             ▼
    ┌────────────────┐
    │ State Vector   │
    │ (64-dim)       │
    │                │
    │ • Price        │
    │ • Volume       │
    │ • Indicators   │
    │ • Position     │
    │ • Portfolio    │
    └────────┬───────┘
             │
             ▼
    ┌────────────────┐
    │ Actor Network  │
    │ (Policy π)     │
    │                │
    │ Input: 64      │
    │ Hidden: 256    │
    │ Output: 1      │
    │ (position %)   │
    └────────┬───────┘
             │
             │ Action
             ▼
    ┌────────────────┐
    │ Execute Trade  │
    │ with Size      │
    └────────┬───────┘
             │
             │ Result
             ▼
    ┌────────────────┐
    │ Reward Calc    │
    │                │
    │ r = profit *   │
    │ (1 - risk) +   │
    │ sharpe_bonus   │
    └────────┬───────┘
             │
             ▼
    ┌────────────────┐
    │ Critic Network │
    │ (Value V)      │
    │                │
    │ Input: 64      │
    │ Hidden: 256    │
    │ Output: 1      │
    │ (value)        │
    └────────┬───────┘
             │
             │ TD Error
             ▼
    ┌────────────────┐
    │ Policy Update  │
    │ (PPO)          │
    │                │
    │ Clip ratio:    │
    │ [0.8, 1.2]     │
    └────────────────┘
```

---

## 6. SIKKERHET OG COMPLIANCE

### 6.1 Sikkerhetslag

```
┌───────────────────────────────────────────────────────────────┐
│                  SECURITY LAYERS                              │
└───────────────────────────────────────────────────────────────┘

Layer 1: Network Security
├── HTTPS/TLS encryption
├── IP whitelist on exchange
├── DDoS protection
└── VPN for VPS access

Layer 2: Application Security
├── JWT authentication
├── API key rotation
├── CORS policy
└── Rate limiting

Layer 3: Data Security
├── Environment variables for secrets
├── Redis password auth
├── Database encryption at rest
└── Secure backup storage

Layer 4: Business Logic Security
├── Risk limits enforcement
├── Emergency stop system
├── Position size limits
└── Drawdown protection

Layer 5: Audit & Monitoring
├── Audit trail logging
├── Alert system
├── Security event monitoring
└── Compliance reporting
```

### 6.2 Authentication Flow

```
┌──────────┐                              ┌──────────┐
│  Client  │                              │ Backend  │
└─────┬────┘                              └─────┬────┘
      │                                         │
      │  1. POST /auth/login                   │
      │    {username, password}                │
      ├────────────────────────────────────────►│
      │                                         │
      │                                         │ 2. Validate
      │                                         │    credentials
      │                                         ├──────┐
      │                                         │      │
      │                                         │◄─────┘
      │                                         │
      │  3. JWT token + refresh token          │
      │◄────────────────────────────────────────┤
      │                                         │
      │  4. Subsequent requests                 │
      │    Authorization: Bearer <JWT>         │
      ├────────────────────────────────────────►│
      │                                         │
      │                                         │ 5. Verify
      │                                         │    JWT
      │                                         ├──────┐
      │                                         │      │
      │                                         │◄─────┘
      │                                         │
      │  6. Response                            │
      │◄────────────────────────────────────────┤
      │                                         │
```

---

## 7. SKALERBARHET OG PERFORMANCE

### 7.1 Horizontal Scaling Strategy

```
┌───────────────────────────────────────────────────────────────┐
│              HORIZONTAL SCALING ARCHITECTURE                  │
└───────────────────────────────────────────────────────────────┘

              ┌─────────────────┐
              │ Load Balancer   │
              │ (Nginx/HAProxy) │
              └────────┬─────────┘
                       │
         ┌─────────────┼─────────────┐
         │             │             │
         ▼             ▼             ▼
┌─────────────┐ ┌─────────────┐ ┌─────────────┐
│ Backend Pod │ │ Backend Pod │ │ Backend Pod │
│     #1      │ │     #2      │ │     #3      │
│             │ │             │ │             │
│ • API       │ │ • API       │ │ • API       │
│ • WebSocket │ │ • WebSocket │ │ • WebSocket │
└─────────────┘ └─────────────┘ └─────────────┘
         │             │             │
         └─────────────┼─────────────┘
                       │
         ┌─────────────┼─────────────┐
         │             │             │
         ▼             ▼             ▼
┌─────────────┐ ┌─────────────┐ ┌─────────────┐
│   Redis     │ │ PostgreSQL  │ │ AI Engine   │
│   Cluster   │ │   Cluster   │ │   Cluster   │
│             │ │             │ │             │
│ • Master    │ │ • Primary   │ │ • Model 1   │
│ • Replica   │ │ • Replica   │ │ • Model 2   │
└─────────────┘ └─────────────┘ └─────────────┘
```

### 7.2 Performance Optimization

**Caching Strategy:**

```
┌───────────────────────────────────────────────────────────────┐
│                   CACHING LAYERS                              │
└───────────────────────────────────────────────────────────────┘

Layer 1: Application Cache (In-Memory)
├── LRU cache for hot data (1000 items)
├── TTL: 60 seconds
└── Use: Frequently accessed data

Layer 2: Redis Cache
├── Distributed cache for all instances
├── TTL: Configurable per key
└── Use: Session data, API responses

Layer 3: Database Query Cache
├── PostgreSQL query result cache
├── Materialized views for analytics
└── Use: Complex aggregations

Layer 4: CDN Cache (Frontend)
├── Static assets (JS, CSS, images)
├── TTL: 1 week
└── Use: Public-facing content
```

**Database Optimization:**

```sql
-- Indexes for performance
CREATE INDEX idx_trades_symbol_created ON trades(symbol, created_at DESC);
CREATE INDEX idx_positions_symbol_status ON positions(symbol, status);
CREATE INDEX idx_signals_created ON signals(created_at DESC);

-- Partitioning for scalability
CREATE TABLE trades_2025_12 PARTITION OF trades
FOR VALUES FROM ('2025-12-01') TO ('2026-01-01');

-- Materialized views for analytics
CREATE MATERIALIZED VIEW daily_performance AS
SELECT
    DATE(created_at) as date,
    COUNT(*) as trades_count,
    SUM(pnl) as total_pnl,
    AVG(roi) as avg_roi
FROM trades
WHERE status = 'CLOSED'
GROUP BY DATE(created_at);
```

---

**Dokumentert av:** GitHub Copilot  
**Dato:** 17. desember 2025  
**Versjon:** 1.0  
**Status:** ✅ KOMPLETT
