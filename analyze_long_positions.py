...
print("\n" + "=" * 100)
print("[CHART] POSITION ANALYSIS - APTUSDT & SOLUSDT")
print("=" * 100)
...
print(f"\n⚙️ System Config: TP={tp_pct*100:.1f}% | SL={sl_pct*100:.1f}% | Trail={trail_pct*100:.1f}% | Partial={partial_tp*100:.0f}%\n")
...
print(f"\n[TARGET] Expected Protection (Our System):")
print(f"   TP Target:   ${apt_tp:>12.6f} (+{tp_pct*100:.1f}%)")
print(f"      → Closes {apt_size * partial_tp:>8,.1f} APT (50%)")
print(f"   SL Target:   ${apt_sl:>12.6f} (-{sl_pct*100:.1f}%)")
print(f"      → Full position")
print(f"   Trail Trig:  ${apt_trail:>12.6f} (+{trail_pct*100:.1f}%)")
print(f"      → For remaining {apt_size * (1-partial_tp):>8,.1f} APT (50%)")

print(f"\n[SEARCH] Binance UI Shows:")
...
if price_move_apt >= trail_pct * 100:
    print(f"   [CHART_UP] Trailing stop should be ACTIVE!")
elif price_move_apt >= 0:
    print(f"   [OK] In profit, watching for trailing activation at +{trail_pct*100:.1f}%")
else:
    print(f"   [WARNING] Currently in loss ({price_move_apt:+.2f}%)")
    print(f"   [WARNING] Will hit SL if price drops to ${apt_sl:.6f} ({dist_to_sl:.2f}% more)")

# ========================================
# SOLUSDT Analysis
# ========================================
print("\n" + "=" * 100)
print("2️⃣ SOLUSDT LONG")
print("=" * 100)
...
print(f"      → Closes {sol_size * partial_tp:>8,.2f} SOL (50%)")
print(f"   SL Target:   ${sol_sl:>12.4f} (-{sl_pct*100:.1f}%)")
print(f"      → Full position")
print(f"   Trail Trig:  ${sol_trail:>12.4f} (+{trail_pct*100:.1f}%)")
print(f"      → For remaining {sol_size * (1-partial_tp):>8,.2f} SOL (50%)")
...
if price_move_sol >= trail_pct * 100:
    ...
    print(f"   [WARNING] Currently in loss ({price_move_sol:+.2f}%)")
    print(f"   [WARNING] Will hit SL if price drops to ${sol_sl:.4f} ({dist_to_sl_sol:.2f}% more)")
...
print(f"\n[BRIEFCASE] Combined Long Positions:")
print(f"   Total Margin:     {total_margin:>10,.2f} BNFCR")
print(f"   Total Unrealized: {total_pnl:>10,.2f} BNFCR ({total_pnl_pct:>+7.2f}%)")

print(f"\n[TARGET] Both positions have:")
print(f"   [OK] Partial TP orders set (50% each)")
print(f"   [OK] Trailing stop orders (remaining 50%)")
print(f"   [OK] Stop loss backup (full position)")
