...
class SystemTestRunner:
    ...
    def check_prerequisites(self) -> Dict[str, Any]:
        ...
        # Summary
        all_scripts_present = all(prereqs["test_scripts_present"].values())
        all_deps_available = all(prereqs["dependencies_available"].values())
    ...
    def run_test_script(self, test_info: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, script_name, "--output", f"results_{script_name}.json"],
                capture_output=True,
                text=True,
            ...
            if detailed_results:
                ...
                if "tests_passed" in detailed_results and "tests_run" in detailed_results:
                    ...
                    if detailed_results["tests_run"] > 0:
                        test_result["success_rate"] = detailed_results["tests_passed"] / detailed_results["tests_run"]
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        test_success_rate = total_tests_passed / total_tests_run if total_tests_run > 0 else 0
        ...
        # Performance summary
        durations = [result.get("duration_ms", 0) for result in self.test_results.values()]
        performance_summary = {
            "total_test_duration_ms": sum(durations),
            "average_category_duration_ms": sum(durations) / len(durations) if durations else 0,
        ...
        # Coverage analysis
        coverage_analysis = {
            "backend_coverage": "backend" in [r.get("script", "").lower() for r in self.test_results.values()],
            "frontend_coverage": "frontend" in [r.get("script", "").lower() for r in self.test_results.values()],
            "integration_coverage": "e2e" in [r.get("script", "").lower() for r in self.test_results.values()],
            "api_testing": any("api" in str(r.get("detailed_results", {})) for r in self.test_results.values()),
            "websocket_testing": any("websocket" in str(r.get("detailed_results", {})) for r in self.test_results.values()),
            "load_testing": any("load" in str(r.get("detailed_results", {})) for r in self.test_results.values())
        }
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        for category, result in self.test_results.items():
            if not result.get("success", False):
                ...
                recommendations.append(f"⚠️ Improve reliability in {category} (current: {result.get('success_rate', 0):.1%})")
        ...
        if avg_duration > 60000:  # > 1 minute average
            recommendations.append("⚡ Consider optimizing test performance - average duration is > 1 minute")
        
        # Coverage recommendations
        coverage_gaps = []
        if not any("backend" in r.get("script", "").lower() for r in self.test_results.values()):
            coverage_gaps.append("backend API testing")
        if not any("frontend" in r.get("script", "").lower() for r in self.test_results.values()):
            coverage_gaps.append("frontend component testing")
        if not any("e2e" in r.get("script", "").lower() for r in self.test_results.values()):
            coverage_gaps.append("end-to-end integration testing")
        
        if coverage_gaps:
            recommendations.append(f"📊 Add missing test coverage: {', '.join(coverage_gaps)}")
        
        # Success recommendations
        overall_success_rate = self.overall_summary.get("overall_success_rate", 0)
        if overall_success_rate >= 0.95:
            recommendations.append("🎉 Excellent test coverage and reliability! System is production-ready.")
        elif overall_success_rate >= 0.85:
            recommendations.append("✅ Good test coverage. Address failing tests for production readiness.")
        elif overall_success_rate >= 0.75:
            recommendations.append("⚠️ Moderate test coverage. Significant improvements needed before production.")
        else:
            recommendations.append("❌ Poor test coverage. Major issues must be resolved before deployment.")
    ...
    def print_test_summary(self, report: Dict[str, Any]):
        ...
        print(f"\n⚡ Performance:")
        print(f"   Average Category Duration: {perf['average_category_duration_ms']:.0f}ms")
