    ...
    async def _handle_trade_intent(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            if not current_price:
                self.logger.error(f"[trade_intent] Failed to get current price | symbol={symbol} trace_id={trace_id}")
                return
            ...
            if side == "LONG":
                ...
                self.logger.warning(f"[trade_intent] Unknown side | side={side} trace_id={trace_id}")
                return
