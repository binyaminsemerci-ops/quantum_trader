...
if not pos:
    print("❌ Ingen NEARUSDT posisjon funnet i Binance API")
    exit(0)
...
print(f"\n[SEARCH] NEARUSDT POSISJON ANALYSE\n{'='*60}")
...
print(f"\n[CHART] Pris bevegelse: {price_change:+.2f}%")
...
print(f"\n[TARGET] FORVENTET TP/SL (basert på QT_SL_PCT=0.02, QT_TP_PCT=0.03):")
print(f"Stop Loss burde være: ${sl_price_expected:.4f} (+2% fra entry)")
print(f"Take Profit burde være: ${tp_price_expected:.4f} (-3% fra entry)")

# Check actual orders
orders = client.futures_get_open_orders(symbol='NEARUSDT')
print(f"\n[CLIPBOARD] FAKTISKE ORDERS ({len(orders)}):")
...
for order in orders:
    ...
    if order_type == 'STOP_MARKET' and order.get('closePosition'):
        ...
        print(f"   🛑 STOP_MARKET (SL): ${stop_price:.4f}")
    elif order_type == 'TAKE_PROFIT_MARKET':
        take_profit_orders.append(stop_price)
        print(f"   [OK] TAKE_PROFIT: ${stop_price:.4f}")
    elif order_type == 'TRAILING_STOP_MARKET':
        trailing_orders.append(stop_price)
        print(f"   [CHART_UP] TRAILING_STOP: ${stop_price:.4f}")

# Analysis
print(f"\n[WARNING]  PROBLEM ANALYSE:")

if not stop_loss_orders:
    print(f"❌ INGEN STOP_MARKET ORDER! Posisjonen er ubeskyttet!")
else:
    sl_actual = stop_loss_orders[0]
    sl_distance_pct = ((sl_actual - entry) / entry) * 100
    print(f"[OK] Stop Loss finnes: ${sl_actual:.4f}")
    print(f"   Distance fra entry: {sl_distance_pct:+.2f}%")
    
    if sl_distance_pct > 2.5:
        print(f"   [ALERT] PROBLEM: SL er {sl_distance_pct:.2f}% over entry (burde være ~2%)")
        print(f"   Med 20x leverage betyr dette {sl_distance_pct * 20:.0f}% margin tap før SL trigger!")
    
    # Check if current price already passed SL
    if mark >= sl_actual:
        print(f"   [ALERT] KRITISK: Mark price ${mark:.4f} er ALLEREDE over SL ${sl_actual:.4f}!")
        print(f"   Stop loss burde ha trigget!")

# Current status
print(f"\n📉 NÅVÆRENDE STATUS:")
print(f"Mark price: ${mark:.4f}")
print(f"Må stige til: ${stop_loss_orders[0] if stop_loss_orders else 'INGEN SL':.4f} for SL trigger")
...
if pnl < 0 and abs(pnl) > 50:  # More than $50 loss
    print(f"\n[ALERT] ANBEFALING: Med ${pnl:.2f} tap ({(pnl/(abs(amt)*entry/leverage))*100:.1f}% av margin)")
    print(f"Vurder å lukke posisjonen manuelt ELLER sett en tettere stop loss!")
