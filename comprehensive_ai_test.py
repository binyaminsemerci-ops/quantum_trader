...
TEST_CONFIGS = {
    ...
    "comprehensive": {
        "symbols": [
            ...
            "AVAXUSDT", "MATICUSDT", "DOTUSDT", "LINKUSDT", "ATOMUSDT", "LTCUSDT"
        ],
        "limit": 1000,
...
class AITestRunner:
    ...
    def run_training_test(self, config_name: str, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = train_and_save(
                ...
                backtest=True,
                write_report=True,
                entry_threshold=config["entry_thresholds"][0]  # Use first threshold for training
            )
    ...
    def run_backtest_variations(self, config_name: str, config: Dict[str, Any], model_dir: Path = None) -> List[Dict[str, Any]]:
        ...
        for threshold in config["entry_thresholds"]:
            ...
            try:
                ...
                duration = time.time() - start_time
                error_result = {"error": str(e), "type": type(e).__name__, "threshold": threshold}
    ...
    def run_model_validation(self, model_dir: Path) -> Optional[Dict[str, Any]]:
        ...
        for file_name in expected_files:
            ...
            if not file_path.exists():
                validation_result["validation_errors"].append(f"Missing artifact: {file_name}")
            ...
            if report:
                ...
                for field in required_fields:
                    if field not in report:
                        validation_result["validation_errors"].append(f"Missing report field: {field}")
                
                # Validate metrics
                if "metrics" in report:
                    metrics = report["metrics"]
                    if "mse" not in metrics:
                        validation_result["validation_errors"].append("Missing MSE metric")
                    if "r2" not in metrics:
                        validation_result["validation_errors"].append("Missing R² metric")
            else:
                validation_result["validation_errors"].append("Could not load training report")
                
        except Exception as e:
            validation_result["validation_errors"].append(f"Report validation error: {e}")
    ...
    def run_performance_comparison(self) -> Dict[str, Any]:
        ...
        for result in self.test_results:
            ...
            if test_type not in comparison["test_breakdown"]:
                comparison["test_breakdown"][test_type] = {"count": 0, "successful": 0, "avg_duration": 0}
            
            comparison["test_breakdown"][test_type]["count"] += 1
            if result["success"]:
                comparison["test_breakdown"][test_type]["successful"] += 1
            comparison["test_breakdown"][test_type]["avg_duration"] += result["duration_seconds"]
        ...
        backtest_results = [r for r in self.test_results if r["test_name"].startswith("backtest_") and r["success"]]
        ...
        if backtest_results:
            ...
            if returns:
                comparison["performance_metrics"]["avg_return"] = sum(returns) / len(returns)
                comparison["performance_metrics"]["best_return"] = max(returns)
                comparison["performance_metrics"]["worst_return"] = min(returns)
            
            if sharpe_ratios:
                comparison["performance_metrics"]["avg_sharpe"] = sum(sharpe_ratios) / len(sharpe_ratios)
                comparison["performance_metrics"]["best_sharpe"] = max(sharpe_ratios)
            
            if win_rates:
                comparison["performance_metrics"]["avg_win_rate"] = sum(win_rates) / len(win_rates)
...
def main():
    parser = argparse.ArgumentParser(description="Comprehensive AI Model Testing for Quantum Trader")
    parser.add_argument("--quick-test", action="store_true", help="Run quick test with 3 symbols")
    parser.add_argument("--standard-test", action="store_true", help="Run standard test with 6 symbols")
    parser.add_argument("--full-test", action="store_true", help="Run comprehensive test with 12 symbols")
    parser.add_argument("--stress-test", action="store_true", help="Run stress test with all symbols")
    parser.add_argument("--compare-configs", action="store_true", help="Run all configurations and compare")
    parser.add_argument("--output-dir", type=Path, help="Output directory for test results")
    
    args = parser.parse_args()
    
    if not any([args.quick_test, args.standard_test, args.full_test, args.stress_test, args.compare_configs]):
