...
**Dependencies:** P3.5 operational (✅ DONE)
...
def _check_alerts(self):
    ...
    try:
        ...
        logger.warning(
            f"🚨 ALERT ({severity}): {reason} @ {share*100:.1f}% "
            f"({int(score)}/{total})"
        )
...
- ✅ Service remains stable (no crashes, no memory leaks)
- ✅ Alerts published when gate exceeds threshold
- ✅ Deduplication works (max 1 alert per reason per 5 min)
- ✅ No false positives (alerts only when truly anomalous)
- ✅ Alert stream TTL capped (maxlen=1000, ~24 hours of history)
...
3. **Alert Trends:** Detect increasing share over time
   - If gate share growing 10%+ per hour → early warning
...
5. **Multi-Window Alerts:** Alert if gate persists across windows
   - Example: Gate >40% in 5m, 15m, AND 1h → definitely stuck
...
|------|----------|--------|
| Code implementation | 20 min | ⏳ PENDING |
| Config updates | 5 min | ⏳ PENDING |
...
**Prerequisites:**
- ✅ P3.5 stable (verified: 14+ hours uptime, 16,300+ messages)
- ✅ Dashboard queries working (tested)
- ✅ Action mapping defined (documented)
