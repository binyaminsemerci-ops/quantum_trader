...
**Timing**: After parsing plan from `quantum:stream:apply.plan`, before execution  
**Trigger**: RECONCILE_CLOSE plans or direct stream injection  
**Use Case**: Deterministic proof testing and reconciliation flows

**Flow**:
```
quantum:stream:apply.plan → Apply consumes → [Observer called here] → execute_reconcile_close()
```
...
- **Cross-flow protection**: Same dedupe key works across both observation points
