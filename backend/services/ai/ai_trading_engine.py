...
class AITradingEngine:
    ...
    def __init__(self, agent=None, db_session=None):
        ...
        # [P0-06] Track model version for diagnostics
        self._model_version = getattr(agent, 'model_version', 'unknown') if agent else 'unknown'
        if agent:
            logger.info(f"[P0-06] AITradingEngine initialized with model version: {self._model_version}")
    ...
    async def get_trading_signals(
        ...
        """
        logger.info("[ROCKET] get_trading_signals called with %d symbols", len(symbols))
        if not self.agent:
        ...
        try:
            # [ROCKET] PARALLEL processing for 222 symbols - process in batches
            import asyncio
            ...
            logger.info("[ROCKET] Processing %d symbols in parallel batches...", len(symbols))
            batch_size = 50
            signals = []
    ...
    async def _get_ensemble_prediction(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Guarantee required model features exist (default 0.0) to avoid missing keys
            required_keys = [
    ...
    def _process_prediction(
        ...
        # [TARGET] HYBRID AI-DRIVEN TP/SL SYSTEM
    ...
    def _calculate_dynamic_tpsl(
        ...
        """
        🔥 RL-UNIFIED TP/SL SYSTEM - Uses RL Agent for learned, adaptive exits
        
        RL Agent learns optimal TP/SL based on:
        ...
        try:
            ...
            if rl_agent:
                # Make a lightweight sizing decision just to get TP/SL strategy
                # Use minimal parameters since we only need TP/SL, not position size
                rl_decision = rl_agent.decide_sizing(
                    symbol="GENERIC",  # Generic symbol for signal generation
                    confidence=confidence,
                    atr_pct=volatility_estimate,
                    current_exposure_pct=0.5,  # Assume moderate exposure
                    equity_usd=1000.0,  # Dummy value, we only need TP/SL %
                    adx=None,
                    trend_strength=None
                )
                
                logger.info(
                    f"🤖 [RL TP/SL] {action}: confidence={confidence:.2f} → "
                    f"TP={rl_decision.tp_percent*100:.1f}% SL={rl_decision.sl_percent*100:.1f}% "
                    f"Strategy={rl_decision.reasoning.split('Strategy=')[1].split(',')[0] if 'Strategy=' in rl_decision.reasoning else 'UNKNOWN'}"
                )
                
                return {
                    "tp_percent": rl_decision.tp_percent,
                    "sl_percent": rl_decision.sl_percent,
                    "trail_percent": rl_decision.partial_tp_percent,  # Use partial TP% as trail
                    "partial_tp": rl_decision.partial_tp_size if rl_decision.partial_tp_enabled else 0.0
                }
            else:
                logger.warning("[RL TP/SL] RL agent not available, using fallback values")
            ...
            logger.error(f"[RL TP/SL] Failed to calculate RL TP/SL: {e}, using fallback")
            # Fallback values
            return {
                "tp_percent": 0.06,
                "sl_percent": 0.025,
    ...
    async def _retrain_model(self, min_samples: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            X_list = []
            y_list = []
            feature_len = None  # enforce consistent feature vector length
            ...
            for sample in samples:
                try:
                    ...
                    # Skip ragged/nested feature shapes
                    if any(isinstance(v, (list, tuple, dict)) for v in feature_array):
                        logger.debug(
                            "Skipping sample %s due to nested feature values", sample.id
                        )
                        continue

                    # Skip empty feature vectors
                    if not feature_array:
                        continue

                    # Establish feature length on first valid sample and skip mismatches
                    if feature_len is None:
                        feature_len = len(feature_array)
                    if len(feature_array) != feature_len:
                        logger.debug(
                            "Skipping sample %s due to feature length mismatch (%s vs %s)",
                            sample.id,
                            len(feature_array),
                            feature_len,
                        )
