...
def _load_model():
    ...
    candidates = [
        os.path.join(os.path.dirname(os.path.dirname(__file__)), 'ai_engine', 'models', 'xgb_model.pkl'),
        os.path.join(os.path.dirname(os.path.dirname(__file__)), '..', 'ai_engine', 'models', 'xgb_model.pkl'),
    ]
    for p in candidates:
        if os.path.exists(p):
    ...
    json_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), '..', 'ai_engine', 'models', 'xgb_model.json')
...
async def reload_model_endpoint():
    ...
    agent = make_default_agent()
    agent.reload()
    return ReloadResponse(loaded_model=(agent.model is not None), loaded_scaler=(agent.scaler is not None))
...
async def train_endpoint(req: TrainRequest, background: BackgroundTasks):
    ...
    symbols = req.symbols or ['BTCUSDT', 'ETHUSDT']
    ...
    return {'status': 'scheduled', 'task_id': task.id, 'symbols': symbols, 'limit': limit}
...
async def list_tasks(limit: int = 50, offset: int = 0):
    ...
    db = next(get_db())
    try:
        q = db.query(TrainingTask).order_by(TrainingTask.created_at.desc()).limit(limit).offset(offset)
        items = []
        for t in q.all():
            items.append({
                'id': t.id,
...
async def status_endpoint():
    ...
    try:
        ...
        meta = None
    return {'loaded_model': agent.model is not None, 'loaded_scaler': agent.scaler is not None, 'metadata': meta}
