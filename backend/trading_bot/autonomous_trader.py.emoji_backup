...
class Position(TypedDict, total=False):
    """Structured type for an open trading position."""
...
class AutonomousTradingBot:
    ...
    def __init__(
        ...
        self.market_balances = {
            market: balance / len(self.enabled_markets)
        ...
        self.positions: Dict[str, Dict[str, Position]] = {
            market: {} for market in self.enabled_markets
        }
        self.daily_trades: Dict[str, int] = {
            market: 0 for market in self.enabled_markets
        }
        ...
        logger.info(
            f"Multi-market trading bot initialized - Markets: {self.enabled_markets}, Dry run: {dry_run}"
        )
    ...
    async def _process_signal(self, signal: Dict[str, Any]):
        ...
        try:
            ...
            base_token = (
                symbol.replace("USDC", "").replace("USDT", "").replace("BUSD", "")
            )
            ...
            if optimal_market not in self.enabled_markets:
                optimal_market = self.enabled_markets[
                    0
                ]  # Fallback to first enabled market

            # Check if we already have position in this symbol across any market
            has_position = any(
                symbol in positions for positions in self.positions.values()
            )
            if has_position:
            ...
            if not current_price:
                logger.warning(
                    f"Could not get price for {market_symbol} on {optimal_market}"
                )
                return
            ...
            stop_loss_price = self._calculate_stop_loss(
                current_price, side, optimal_market
            )
            ...
            if not is_valid:
                logger.warning(
                    f"Trade rejected by risk manager for {optimal_market}: {reason}"
                )
                return
    ...
    def _calculate_take_profit(
        self, price: float, side: str, market_type: str
    ) -> float:
    ...
    async def _execute_trade(
        ...
        try:
            if self.dry_run:
                ...
                logger.info(
                    f"🔥 DRY RUN TRADE [{market_type}]: {side.upper()} {qty} {symbol} @ {price} (confidence: {confidence})"
                )
            ...
            await self._save_trade_to_db(
                symbol, side, qty, price, confidence, original_signal, market_type
            )
    ...
    async def _save_trade_to_db(
        ...
        try:
            ...
            db_path = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "trades.db"
            )
                ...
                try:
                    ...
                    if self._should_close_position(position, current_price):
                        positions_to_close.append(
                            (market_type, symbol, position, current_price)
                        )

                except Exception as e:
                    logger.error(
                        f"Error managing position {symbol} in {market_type}: {e}"
                    )
    ...
    def _should_close_position(self, position: Position, current_price: float) -> bool:
        ...
        if side == "buy" and current_price <= stop_loss:
            logger.info(
                f"Stop loss triggered for BUY position: {current_price} <= {stop_loss}"
            )
            return True
        elif side == "sell" and current_price >= stop_loss:
            logger.info(
                f"Stop loss triggered for SELL position: {current_price} >= {stop_loss}"
            )
            return True
        ...
        if take_profit:
            if side == "buy" and current_price >= take_profit:
                logger.info(
                    f"Take profit triggered for BUY position: {current_price} >= {take_profit}"
                )
                return True
            elif side == "sell" and current_price <= take_profit:
                logger.info(
                    f"Take profit triggered for SELL position: {current_price} <= {take_profit}"
                )
                return True
        ...
        max_hold_hours = (
            2 if market_type in ["FUTURES", "MARGIN", "CROSS_MARGIN"] else 4
        )

        if datetime.now() - entry_time > timedelta(hours=max_hold_hours):
            logger.info(
                f"Time-based exit triggered for {market_type} position after {max_hold_hours}h"
            )
            return True
    ...
    async def _close_position(
        ...
        try:
            ...
            if self.dry_run:
                logger.info(
                    f"🔥 DRY RUN CLOSE [{market_type}]: {side.upper()} {qty} {symbol} @ {current_price}"
                )
            else:
                logger.info(
