...
class SystemTestSummaryGenerator:
    ...
    def check_system_components(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "backend_ready": components["backend_directory"] and (components["backend_main"] or components["backend_simple"]),
            "frontend_ready": components["frontend_directory"] and components["frontend_package"],
    ...
    def analyze_test_coverage(self) -> Dict[str, Any]:
        ...
        if "backend" in self.results and self.results["backend"].get("detailed_results"):
        ...
        if "frontend" in self.results and self.results["frontend"].get("detailed_results"):
    ...
    def calculate_system_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self.generate_health_recommendations(weighted_score, components, coverage)
        }
    
    def generate_health_recommendations(self, overall_score: float, components: Dict[str, Any], coverage: Dict[str, Any]) -> List[str]:
        """Generate recommendations based on system health."""
        recommendations = []
        
        # Structure recommendations
        if not components["backend_ready"]:
            recommendations.append("🔧 Fix backend structure: Ensure backend directory has working main.py or simple_main.py")
        
        if not components["frontend_ready"]:
            recommendations.append("🔧 Fix frontend structure: Ensure frontend directory has valid package.json")
        ...
        if test_gaps:
            recommendations.append(f"📊 Add missing test coverage: {', '.join(test_gaps)}")
        
        # Score-based recommendations
        if overall_score >= 0.8:
            recommendations.append("🎉 Excellent system health! Ready for production deployment.")
        elif overall_score >= 0.6:
            recommendations.append("✅ Good system health. Address minor issues for production readiness.")
        elif overall_score >= 0.4:
            recommendations.append("⚠️ Fair system health. Significant improvements needed before production.")
        else:
            recommendations.append("❌ Poor system health. Major issues must be resolved before deployment.")
        
        # Specific technical recommendations
        if "backend" in self.results:
            backend_result = self.results["backend"]
            if backend_result.get("success_rate", 0) < 0.5:
                recommendations.append("🔧 Fix backend connectivity issues - server may not be starting properly")
        ...
        if "frontend" in self.results:
            ...
            if "detailed_results" in frontend_result:
                for result in frontend_result["detailed_results"]:
                    if result.get("test_name") == "npm_available" and not result.get("success"):
                        recommendations.append("🛠️ Install Node.js and npm for frontend development")
        ...
        for category, results in self.results.items():
            if "error" not in results:
                test_summaries[category] = {
                    ...
                    "duration_ms": results.get("total_duration_ms", results.get("test_duration_ms", 0)),
                    "status": "✅ PASS" if results.get("success_rate", 0) >= 0.5 else "❌ FAIL"
                }
            else:
                test_summaries[category] = {
                    "status": "❌ ERROR",
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        coverage_items = [
            ...
            ("Frontend Structure", coverage["coverage_details"]["frontend_structure_tested"]),
        ...
        for category, summary in test_summaries.items():
            ...
            if "error" in summary:
                ...
                print(f"       Tests: {summary['tests_passed']}/{summary['tests_run']} passed ({summary['success_rate']:.1%})")
