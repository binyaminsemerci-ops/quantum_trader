...
async def startup():
    ...
    try:
        ...
        # Discover existing positions from Binance
        logger.info("🔍 Discovering existing positions from Binance...")
        try:
            ...
            for pos in active_positions:
                ...
                tracked_positions[symbol] = TrackedPosition(
                    ...
                    take_profit=None,
                    stop_loss=None,
                    order_id=f"DISCOVERED_{symbol}",
                    opened_at=datetime.utcnow().isoformat()
                )
                
                logger.info(f"📊 Discovered: {symbol} {side} {leverage}x - Entry: {entry:.6f}")
            
            logger.info(f"✅ Discovered {len(active_positions)} existing positions")
        except Exception as e:
            logger.warning(f"⚠️  Position discovery failed: {e}")
