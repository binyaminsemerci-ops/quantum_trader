...
**Objective:** Understand why K > 0.6 for ETHUSDT/SOLUSDT → FULL_CLOSE_PROPOSED  
...
|---------|----------------------|-------|-------|--------|
| BTCUSDT | PARTIAL_75          | 7.01  | 0.527 | ✅ HEALTHY (below 0.6 threshold) |
| ETHUSDT | FULL_CLOSE_PROPOSED | 9.8   | 0.753 | ⚠️ KILL TRIGGERED (above 0.6) |
| SOLUSDT | FULL_CLOSE_PROPOSED | 9.8   | 0.761 | ⚠️ KILL TRIGGERED (above 0.6) |
...
kill_score:           0.527 (✅ below 0.6 threshold)
...
### ETHUSDT (⚠️ Kill Triggered)
**MarketState:**
```
sigma:      0.0052    (low volatility)
mu:         0.0005
ts:         0.092     (⚠️ LOW trend strength)
p_trend:    0.247     (weak trend)
p_mr:       0.126     (low mean reversion)
p_chop:     0.627     (⚠️ HIGH chopiness)
```
...
kill_score:           0.753 (⚠️ above 0.6 threshold)
R_net:                9.8 (excellent)
...
**Analysis:**
- K=0.753 > 0.6 → Kill score triggered ✅
- **LOW ts=0.092** (vs ref ~0.3) → ts_drop component high
- **HIGH p_chop=0.627** (trend→chop transition) → regime_flip component high
...
### SOLUSDT (⚠️ Kill Triggered)
**MarketState:**
```
sigma:      0.00015   (very low volatility)
mu:         0.000002
ts:         0.014     (⚠️ VERY LOW trend strength)
p_trend:    0.235     (weak trend)
p_mr:       0.115     (low mean reversion)
p_chop:     0.649     (⚠️ VERY HIGH chopiness)
```
...
```
harvest_action:       FULL_CLOSE_PROPOSED
kill_score:           0.761 (⚠️ above 0.6 threshold)
R_net:                9.8 (excellent)
new_sl_proposed:      50.1
reason_codes:         harvest_partial_75, profit_lock,
                      kill_score_triggered, regime_flip, ts_drop
...
**Analysis:**
- K=0.761 > 0.6 → Kill score triggered ✅
- **VERY LOW ts=0.014** (vs ref ~0.3) → ts_drop component VERY high
- **VERY HIGH p_chop=0.649** (strong chop regime) → regime_flip component high
...
**Result:** ❌ Component breakdown fields **NOT present** in Redis
...
- `regime_flip`: 0 or 1 (trend→chop/mr flip)
...
1. **regime_flip** (0 or 1):
   ...
   - ETHUSDT: p_trend=0.247 < 0.3, (p_chop+p_mr)=0.753 > 0.5 → **regime_flip = 1.0** ✅
   - SOLUSDT: p_trend=0.235 < 0.3, (p_chop+p_mr)=0.764 > 0.5 → **regime_flip = 1.0** ✅

2. **sigma_spike** (0 to cap):
   - `max(0, min(sigma/sigma_ref - 1, cap))`
   - ETHUSDT: sigma=0.0052, ref=0.01 → ratio=0.52 → spike = max(0, 0.52-1) = 0 ✅
   - SOLUSDT: sigma=0.00015, ref=0.01 → ratio=0.015 → spike = 0 ✅

3. **ts_drop** (0 to cap):
   - `max(0, min(ts_ref - ts, cap))`
   - ETHUSDT: ts=0.092, ref=0.3 → drop = min(0.3-0.092, 0.5) = **0.208** ✅
   - SOLUSDT: ts=0.014, ref=0.3 → drop = min(0.3-0.014, 0.5) = **0.286** ✅

4. **age_penalty** (0 to 1):
   - `min(age_sec / max_age_sec, 1)`
   - ETHUSDT: age=3600, max=86400 → penalty = 3600/86400 = **0.042** ✅
   - SOLUSDT: age=3600, max=86400 → penalty = **0.042** ✅
...
- `regime_flip` → present in codes ✅
- `ts_drop` → present in codes ✅
- `sigma_spike` → NOT in codes (confirms sigma_spike low)
- `age_penalty` → NOT in codes (confirms age_penalty low)
...
- regime_flip present → component > 0 ✅
- ts_drop present → component > 0.2 ✅
- sigma_spike NOT present → component <= 0.5 ✅
- age_penalty NOT present → component <= 0.5 ✅

**Conclusion:** K-scores are correct. The edge has collapsed on both symbols due to:
1. **Regime flip** (trend→chop transition)
...
**Benefits:**
- ✅ Minimal change (calc-only, no logic change)
- ✅ Components visible in Redis immediately
- ✅ Dashboard can show component bars
- ✅ No new keys (stays in same hash)
...
**Benefits:**
- ✅ Keeps main hash clean
- ✅ Can include more audit data (tranche_weights, etc.)
- ✅ Optional via env flag

**Drawbacks:**
- ❌ Extra Redis keys (3x more keys)
- ❌ Dashboard needs to join hashes
...
- ❌ Hard to debug high K values
- ❌ Cannot tune theta weights without manual log analysis
...
2. Deploy to VPS (/tmp → cp → systemctl restart)
...
- regime_flip correctly identifies trend→chop transitions
...
|--------|---------|---------|---------|--------|
| **K-score** | 0.527 | 0.753 | 0.761 | ✅ Correct |
| **Action** | PARTIAL_75 | FULL_CLOSE | FULL_CLOSE | ✅ Correct |
| **regime_flip** | 0 (trending) | 1 (flipped) | 1 (flipped) | ✅ Accurate |
| **sigma_spike** | ~0 (normal) | 0 (low vol) | 0 (low vol) | ✅ Accurate |
| **ts_drop** | ~0.05 (stable) | 0.208 (drop) | 0.286 (drop) | ✅ Accurate |
| **age_penalty** | 0.042 (young) | 0.042 (young) | 0.042 (young) | ✅ Accurate |
| **Components Published** | ❌ No | ❌ No | ❌ No | 🔧 Fix in P2.6B |
...
- ✅ K-scores accurately reflect edge collapse conditions
- ✅ ETHUSDT/SOLUSDT correctly flagged for FULL_CLOSE due to regime_flip + ts_drop
- ✅ BTCUSDT correctly allowed PARTIAL_75 (stable trending)
- ✅ No theta tuning needed at this time

**P2.5 Publisher gap:**
- ❌ k_components not published to Redis (only aggregate kill_score)
- 🔧 Fix: Add 4 fields to hash (regime_flip, sigma_spike, ts_drop, age_penalty)
