...
async def test_futures_api():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            print(f"[OK] Fetched {len(records_list)} liquidity records\n")
            
            # Show first 10 records
            print("[CHART] Top 10 symbols by volume:")
            print("-" * 60)
            ...
            # Check quote assets
            print("\n[MONEY] Quote asset distribution:")
            print("-" * 60)
            ...
            if len(quote_counts) == 2 and "USDT" in quote_counts and "USDC" in quote_counts:
                print("\n  [OK] Perfect: Only USDT and USDC pairs")
            elif len(quote_counts) > 2:
                other_quotes = [q for q in quote_counts.keys() if q not in ["USDT", "USDC"]]
                print(f"\n  [WARNING]  Found unexpected quote assets: {', '.join(other_quotes)}")
            
            # Check provider label
            providers = set(record.provider for record in records_list)
            print(f"\n🏷️  Provider labels: {', '.join(providers)}")
            
            if "binance-futures" in providers:
                print("  [OK] Correct: Labeled as 'binance-futures'")
            elif "binance" in providers:
                print("  [WARNING]  Warning: Labeled as 'binance' (should be 'binance-futures')")
            
            print("\n" + "="*60)
            print("[OK] API test completed successfully!")
            print("="*60 + "\n")
