...
if not api_key or not api_secret:
    print("❌ BINANCE_API_KEY or BINANCE_API_SECRET not found in environment")
    exit(1)

print(f"\n[MEMO] API Key: {api_key[:10]}...{api_key[-4:]}")
print("\nTesting Futures API...")
...
try:
    ...
    try:
        ...
        print(f"[OK] Futures account access OK!")
        ...
        if open_positions:
            ...
            print(f"\n[CHART] No open positions")
        
        if available < 11:
            print(f"\n[WARNING]  USDT: ${available:.2f} (trenger min $11 for trading)")
        else:
            print(f"\n[OK] Balance OK for trading (min $11)")
        ...
        if e.code == -2015:
            ...
            print("   - Gå til: https://www.binance.com/en/my/settings/api-management")
            print("   - Aktiver 'Enable Futures' på API keyen")
        elif e.code == -4028:
            print("🔧 LØSNING:")
            print("   - Futures konto ikke aktivert")
            print("   - Gå til: https://www.binance.com/en/futures/BTCUSDT")
            print("   - Aktiver futures trading først")
