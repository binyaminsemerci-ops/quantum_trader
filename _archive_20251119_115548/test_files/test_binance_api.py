...
try:
    ...
    print('[OK] Binance API connection successful!')
    print(f'Can Trade: {account["canTrade"]}')
    
    # Check balances
    balances = [b for b in account['balances'] if float(b['free']) > 0 or float(b['locked']) > 0]
    print(f'\n[MONEY] Available Balances:')
    ...
    if usdt_bal:
        ...
        print(f'\n💵 USDT Balance: ${usdt_free:.2f} (locked: ${usdt_locked:.2f})')
        if usdt_free < 15:
            print('[WARNING]  WARNING: USDT balance is below minimum notional ($15)')
        else:
            print(f'[OK] Sufficient USDT for trading (min: $15)')
