...
async def set_leverage():
    ...
    print(f"\n🔧 Setting 10x leverage for {len(positions)} positions...\n")
    ...
    for symbol in positions.keys():
        try:
            ...
            print(f"[OK] {symbol}: Leverage set to 10x")
            success_count += 1
        except Exception as e:
            print(f"❌ {symbol}: Failed - {e}")
    
    print(f"\n[OK] Successfully set 10x leverage on {success_count}/{len(positions)} positions")
    
    # Verify by checking account
    print("\n[CHART] Verifying leverage settings...\n")
