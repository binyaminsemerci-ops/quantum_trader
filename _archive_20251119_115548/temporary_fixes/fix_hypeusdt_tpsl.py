...
# Get HYPEUSDT position
positions = client.futures_position_information(symbol='HYPEUSDT')
pos = next((p for p in positions if float(p['positionAmt']) != 0), None)

if not pos:
    print("❌ No HYPEUSDT position found")
    exit()
...
print(f"HYPEUSDT {side}: Size={abs(qty)}, Entry=${entry}")

# Get precision
info = client.futures_exchange_info()
symbol_info = next(s for s in info['symbols'] if s['symbol'] == 'HYPEUSDT')
