...
for symbol, state in trade_state.items():
    ...
    try:
        ...
        if side == "LONG":
            ...
            print(f"  [TARGET] Trail Stop would be: ${trail_stop:.4f} ({trail_pct*100:.1f}% below peak)")
            
            if current_price < trail_stop:
                print(f"  [WARNING]  TRAILING STOP WOULD EXIT! (Price below ${trail_stop:.4f})")
            else:
                distance = ((current_price - trail_stop) / current_price) * 100
                print(f"  [OK] Safe: {distance:.2f}% above trail stop")
            ...
            print(f"  [TARGET] Trail Stop would be: ${trail_stop:.4f} ({trail_pct*100:.1f}% above trough)")
            
            if current_price > trail_stop:
                print(f"  [WARNING]  TRAILING STOP WOULD EXIT! (Price above ${trail_stop:.4f})")
            else:
                distance = ((trail_stop - current_price) / current_price) * 100
                print(f"  [OK] Safe: {distance:.2f}% below trail stop")
...
print("\n" + "=" * 80)
print("\n[WARNING]  NOTE: Trailing stops are NOT currently active!")
