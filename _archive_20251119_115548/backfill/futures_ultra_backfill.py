"""
FUTURES ULTRA BACKFILL - Extended timeframes and periods
Target: Additional 50,000+ futures samples
Multiple timeframes: 1m, 3m, 5m, 15m, 30m, 1h, 4h
Extended periods: Up to 60 days back
"""
import sys
...
def determine_action_outcome(features, future_price, current_price):
    """Futures-optimized action determination"""
    rsi = features['RSI']
    macd = features['MACD']
    ...
    # Ultra aggressive for futures
    if rsi < 47 and macd > 0 and ema_10 > ema_50:
        action = "BUY"
    elif rsi > 53 and macd < 0 and ema_10 < ema_50:
        action = "SELL"
    elif rsi < 33 or close < bb_lower * 1.008:
        action = "BUY"
    elif rsi > 67 or close > bb_upper * 0.992:
        action = "SELL"
...
async def process_symbol(session, symbol, interval, start_time, db, source_name, samples_per_symbol=200):
    """Process one symbol+interval combo"""
    ...
    samples_created = 0
    lookback = 10 if interval in ['1m', '3m', '5m'] else 12 if interval in ['15m', '30m'] else 8
    step = 5 if interval in ['1m', '3m'] else 6 if interval == '5m' else 8
...
async def main(target_samples=50000):
    """Ultra futures backfill with extended coverage"""
    
    print("[ROCKET][ROCKET][ROCKET] FUTURES ULTRA BACKFILL - EXTENDED COVERAGE [ROCKET][ROCKET][ROCKET]")
    print("=" * 80)
    print(f"[TARGET] Target: {target_samples:,} additional futures samples")
    print("[CHART] Multiple timeframes: 1m, 3m, 5m, 15m, 30m, 1h, 4h")
    print("📅 Extended periods: Up to 60 days back")
    print("💪 50 top futures pairs")
    print("=" * 80)
    ...
    # Extended periods with varied timeframes
    configs = [
        # Short-term high frequency
        ('ultra_1m_recent', now - timedelta(days=2), '1m', 200, 8),
        ('ultra_3m_week1', now - timedelta(days=5), '3m', 180, 10),
        
        # Medium-term analysis
        ('ultra_5m_week2', now - timedelta(days=10), '5m', 160, 12),
        ('ultra_15m_week3', now - timedelta(days=17), '15m', 140, 15),
        ('ultra_30m_week4', now - timedelta(days=24), '30m', 120, 18),
        
        # Long-term patterns
        ('ultra_1h_month1', now - timedelta(days=35), '1h', 100, 20),
        ('ultra_4h_month2', now - timedelta(days=50), '4h', 80, 25),
    ]
    ...
    async with aiohttp.ClientSession() as session:
        for source_name, start_time, interval, samples_per_sym, batch_size in configs:
            if total_samples >= target_samples:
                break
            
            print(f"\n[CHART] {source_name} ({interval} from {start_time.strftime('%Y-%m-%d')})")
            config_samples = 0
            valid_symbols = 0
            ...
            for i in range(0, len(futures_symbols), batch_size):
                ...
                tasks = [
                    process_symbol(session, sym, interval, start_time, db, source_name, samples_per_sym)
                    for sym in batch
                ]
                ...
                print(f"   [OK] Processed {min(i+batch_size, len(futures_symbols))}/{len(futures_symbols)}: {config_samples:,} samples...")
            
            print(f"   [OK] Config complete: {config_samples:,} samples from {valid_symbols} pairs")
    ...
    print("\n" + "=" * 80)
    print(f"🎉🎉🎉 FUTURES ULTRA BACKFILL COMPLETE! 🎉🎉🎉")
    print(f"   Total: {total_samples:,} additional FUTURES samples")
    print(f"   Timeframes: 7 different intervals (1m → 4h)")
    print(f"   Coverage: 60 days of futures history")
    print(f"   Leverage-optimized patterns")
    print("=" * 80)
