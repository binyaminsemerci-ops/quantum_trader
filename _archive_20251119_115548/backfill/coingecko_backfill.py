...
async def main(target_samples=50000):
    ...
    print("[CHART] Source: CoinGecko API (aggregated from 600+ exchanges)")
        ...
        for source_name, days, max_samples in configs:
            ...
            for i, (coin_id, symbol) in enumerate(coins, 1):
                ...
                if count > 0:
                    ...
                    total_samples += count
                    print(f"   [OK] {i}/{len(coins)} {symbol}: {count} samples (total: {config_samples:,})...")
                
                if total_samples >= target_samples:
                    print(f"\n[TARGET] TARGET REACHED!")
                    break
            
            print(f"   [OK] Period complete: {config_samples:,} samples from {valid_coins} coins")
