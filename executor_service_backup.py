...
class AutoExecutor:
    ...
    def place_order(
        ...
        try:
            ...
            # Get current price to convert USDT to contract quantity
            ticker = client.futures_symbol_ticker(symbol=symbol)
            current_price = float(ticker['price'])
            
            # Convert USDT position size to contract quantity
            # qty parameter is in USDT, we need contracts
            contract_qty = qty / current_price
            ...
            if symbol_info:
                ...
                for filter_item in symbol_info['filters']:
                    if filter_item['filterType'] == 'LOT_SIZE':
                        ...
                        contract_qty = round(contract_qty, precision)
                        break
            else:
                # Default to 3 decimals
                contract_qty = round(contract_qty, 3)
            
            logger.info(f"📊 Converting {qty} USDT @ ${current_price} = {contract_qty} {symbol} contracts")
            
            # Set leverage
            client.futures_change_leverage(symbol=symbol, leverage=leverage)
            
            # Place market order with contract quantity
            if side.upper() == "BUY":
            ...
            logger.info(f"✅ Order placed: {symbol} {side} {contract_qty} contracts ({qty} USDT) @ {leverage}x")
            ...
            try:
                ...
                # Dynamic TP/SL based on leverage and volatility
                ...
                if leverage >= 5:
                    ...
                    sl_pct = 0.008  # 0.8% SL for high leverage
                ...
                price_precision = len(str(current_price).split('.')[-1]) if '.' in str(current_price) else 2
                ...
                # Don't fail the order if TP/SL fails
    ...
    def get_live_signals(self) -> List[Dict]:
        """Fetch live trading signals from Redis"""
        try:
            # First, try to get signals from trading bot's recent signals list
            bot_signals_raw = r.lrange("bot:latest_signals", 0, 49)  # Get last 50 signals
            
            if bot_signals_raw:
                signals = []
                for signal_json in bot_signals_raw:
                    try:
                        signal = json.loads(signal_json)
                        # Convert bot signal format to executor format
                ...
                if signals:
                    logger.debug(f"📥 Got {len(signals)} signals from trading bot")
                    return signals
            
            # Fallback: use static live_signals if bot signals unavailable
            signals_json = r.get("live_signals")
            if not signals_json:
                return []
            ...
            logger.debug(f"📥 Using fallback signals: {len(signals)} signals")
            return signals
            
        except json.JSONDecodeError:
            logger.error("❌ Invalid JSON in live_signals")
            return []
        except Exception as e:
