...
class EventBusClient:
            ...
            try:
                ...
                for stream_name, stream_messages in messages:
                    for message_id, fields in stream_messages:
                        # Parse JSON payload (but don't ACK yet - let caller do it)
                        payload = None
                        try:
                            if "data" in fields:
                            ...
                            logger.error(f"❌ JSON decode error in {message_id}: {e}")
                            # Poison message - ACK it to avoid infinite loop
                            await self.redis.xack(stream_name, group_name, message_id)
                            continue
                        
                        # Attach metadata for ACK callback
                        if payload:
                            ...
                            # No payload - ACK to avoid poison loop
                            await self.redis.xack(stream_name, group_name, message_id)
    ...
    async def ack_message(self, stream_name: str, group_name: str, message_id: str):
        """
        Acknowledge a message after successful processing by caller.
        This MUST be called by the consumer after confirming the message was handled.
        
        Args:
            stream_name: Redis stream name (e.g., "quantum:stream:trade.intent")
            group_name: Consumer group name
            message_id: Message ID from XREADGROUP
        """
        try:
            await self.redis.xack(stream_name, group_name, message_id)
            logger.debug(f"✅ ACKed {message_id} from {group_name}")
        except Exception as e:
