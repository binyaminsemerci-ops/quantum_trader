#!/usr/bin/env python3
"""
[TEST_TUBE] HYBRID AGENT DRY-RUN TEST SCRIPT
...
def check_paper_trading_mode():
    ...
    if paper_mode:
        print_success("Paper trading mode: ENABLED [OK]")
    else:
        print_error("Paper trading mode: DISABLED [WARNING]")
...
def check_hybrid_agent_loaded():
    ...
    checks = {
        "Hybrid Agent": "Hybrid Agent loaded" in logs or "🤖" in logs,
        "TFT Model": "TFT model loaded" in logs or "[OK] TFT" in logs,
...
def monitor_ai_signals(duration=60):
    ...
    if "Feature shape mismatch" in logs:
        print_error("[WARNING] FEATURE MISMATCH ERROR DETECTED!")
    ...
    if "No module named 'torch'" in logs or "pytorch" in logs.lower():
        print_error("[WARNING] PyTorch import error detected!")
        return False
    else:
...
def check_positions():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"\n[BRIEFCASE] Current Positions: {len(positions)}")
...
def run_full_test():
    ...
    print_header("[TEST_TUBE] HYBRID AGENT DRY-RUN TEST SUITE")
    ...
    if passed == total:
        ...
        print(f"\n{Colors.CYAN}🎉 System is ready for dry-run testing!{Colors.RESET}")
        ...
        print(f"{Colors.YELLOW}MOSTLY PASSED [WARNING]{Colors.RESET}")
        print(f"\n{Colors.YELLOW}System is functional but some issues detected{Colors.RESET}")
    else:
        print(f"{Colors.RED}MULTIPLE FAILURES ❌{Colors.RESET}")
