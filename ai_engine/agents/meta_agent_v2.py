...
class MetaAgentV2:
    ...
    def __init__(
        ...
        # Learning Cadence context (read-only observational signal)
        self.learning_readiness_api = os.getenv(
            "LEARNING_CADENCE_API",
            "http://127.0.0.1:8003"
        )
        self.learning_readiness_status: Optional[Dict[str, Any]] = None
        self.learning_readiness_last_check: Optional[datetime] = None
        self.learning_readiness_check_interval = 600  # 10 minutes
        ...
        logger.info(f"[MetaV2] Learning Cadence API: {self.learning_readiness_api}")
        
        # Initial learning readiness check (non-blocking)
        self._fetch_learning_readiness()
    ...
    def _fetch_learning_readiness(self) -> None:
        ...
        It's purely for context awareness and logging.
        
        Graceful fallback: If API is down, simply logs warning and continues.
        """
        now = datetime.now()
        
        # Rate limiting: Only check every N seconds
        if self.learning_readiness_last_check is not None:
            elapsed = (now - self.learning_readiness_last_check).total_seconds()
            if elapsed < self.learning_readiness_check_interval:
                return  # Too soon, skip check
        
        try:
            response = requests.get(
                f"{self.learning_readiness_api}/readiness/simple",
                timeout=2.0  # Fast timeout, don't block trading
            )
            
            if response.status_code == 200:
                data = response.json()
                self.learning_readiness_status = data
                self.learning_readiness_last_check = now
                ...
                logger.info(
                    f"[META-V2] {status_emoji} Learning readiness: {'READY' if ready else 'NOT_READY'} "
                    f"({reason}) - Actions: {actions}"
                )
            else:
                logger.warning(
                    f"[META-V2] Learning Cadence API returned {response.status_code}"
                )
                
        except requests.exceptions.RequestException as e:
            # Graceful fallback: API down is not a blocker
            logger.debug(
                f"[META-V2] Learning Cadence API unavailable: {e} "
                f"(graceful fallback active)"
            )
        except Exception as e:
            logger.debug(f"[META-V2] Learning readiness check failed: {e}")
    ...
    def predict(
        ...
        # Periodically refresh learning readiness context (non-blocking)
        self._fetch_learning_readiness()
        
        # Log current learning context (observational only)
        if self.learning_readiness_status is not None:
            ready = self.learning_readiness_status.get('ready', False)
            reason = self.learning_readiness_status.get('reason', 'unknown')
            if self.total_predictions % 50 == 1:  # Log every 50 predictions
                logger.info(
                    f"[META-V2] Data context: Learning {'READY' if ready else 'NOT_READY'} ({reason})"
                )
        ...
        # ====================================================================
        # RULE-BASED POLICY CHECKS (No trained model required)
        # ====================================================================
        
        # Check 1: Strong consensus → DEFER (respect base agreement)
        ...
        # Check 2: Analyze disagreement
        disagreement_metrics = self._analyze_disagreement(base_predictions)
        
        # Check 3: Split vote (2 BUY, 2 SELL) → ESCALATE
        ...
        # Check 4: High disagreement (no clear majority) → ESCALATE
        ...
        # Check 5: Low ensemble confidence → ESCALATE
        ...
        # ====================================================================
        # MODEL-BASED UNCERTAINTY CHECK (Optional, requires trained model)
        # ====================================================================
        
        # If model not ready, defer to ensemble (after rule-based checks passed)
        if not self.is_ready():
            result['reason'] = "clear_ensemble_no_model"
            self.fallback_reasons['model_not_loaded'] += 1
            logger.debug(
                f"[MetaV2-Policy]{symbol_str} DEFER: clear ensemble "
                f"(no model, but passed all rule checks)"
            )
            return result
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Include learning readiness context (observational)
        if self.learning_readiness_status is not None:
            stats["learning_readiness"] = {
                "ready": self.learning_readiness_status.get('ready', False),
                "reason": self.learning_readiness_status.get('reason', 'unknown'),
                "allowed_actions": self.learning_readiness_status.get('actions', []),
                "last_checked": self.learning_readiness_last_check.isoformat() if self.learning_readiness_last_check else None
            }
        else:
            stats["learning_readiness"] = {
                "ready": None,
