...
class XGBAgent:
    ...
    # dynamic view can cast later when required.
        ...
        if sentiment_series is not None or news_counts is not None:
            feat = _add_sentiment_features(feat, sentiment_series=sentiment_series, news_counts=news_counts)
        ...
        for s, df in symbol_ohlcv.items():
            try:
                ...
                if hasattr(df, 'columns'):
                    ...
                    if 'volume' in cols:
                        vol = float(df['volume'].iloc[-1]) if 'volume' in df.columns else float(df['Volume'].iloc[-1])
                    else:
                        vol = 0.0
                else:
    ...
    async def scan_top_by_volume_from_api(self, symbols: List[str], top_n: int = 10, limit: int = 240) -> Dict[str, Dict[str, Any]]:
