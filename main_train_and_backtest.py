...
def build_parser() -> argparse.ArgumentParser:
    ...
    train_parser = sub.add_parser(
        "train",
        help="Train a model and optionally run a backtest.",
    )
    train_parser.add_argument(
        "--symbols",
        nargs="+",
        help="Symbols to pull data for (defaults to config)",
    )
    train_parser.add_argument(
        "--limit",
        type=int,
        default=600,
        help="Number of candles per symbol",
    )
    train_parser.add_argument(
        "--model-dir",
        type=Path,
    ...
    train_parser.add_argument(
        "--no-report",
        action="store_true",
        help="Do not write training_report.json",
    )
    train_parser.set_defaults(func=_handle_train)

    backtest_parser = sub.add_parser(
        "backtest",
        help="Run a backtest using existing artifacts.",
    )
    ...
    backtest_parser.add_argument(
        "--model-dir",
        type=Path,
        help="Directory with previously saved artifacts",
    )
    backtest_parser.add_argument(
        "--entry-threshold",
        type=float,
    ...
    report_parser.add_argument(
        "--json",
        action="store_true",
        help="Emit the raw JSON instead of a summary",
    )
