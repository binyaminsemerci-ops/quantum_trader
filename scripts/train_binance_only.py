...
def calculate_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        logger.error(f"Failed to calculate indicators: missing columns {missing}")
        return df
    ...
    try:
        ...
        if 'close' not in df.columns:
            logger.error("Failed to create labels: missing column 'close'")
            return df
...
def fetch_training_data(symbols: List[str], limit: int = 500) -> Dict[str, pd.DataFrame]:
    ...
    for symbol in symbols:
        try:
            ...
            missing = [c for c in REQUIRED_COLS if c not in df.columns]
            if missing:
                logger.error(f"{symbol}: missing expected columns {missing}, skipping symbol")
                continue
            ...
            if 'label' not in df.columns:
                logger.warning(f"{symbol}: no labels generated, skipping symbol")
                continue
            ...
            if len(df) > 50:  # Minimum samples required
                ...
                logger.warning(f"[WARNING] {symbol}: Only {len(df)} samples (need 50+)")
...
def train_xgboost_model(data: Dict[str, pd.DataFrame]) -> tuple:
    ...
    for symbol, df in data.items():
        if 'label' not in df.columns:
            logger.warning(f"{symbol}: missing 'label' column, skipping")
            continue
    ...
    if not all_features:
        logger.error("No training samples collected (no labels/features)")
        sys.exit(1)
...
def save_models(model, scaler, feature_names: List[str]):
    ...
    with open(model_path, 'wb') as f:
        pickle.dump(model, f)
    logger.info(f"[OK] Saved XGBoost model: {model_path}")
...
def main():
    ...
    if csv_path.exists():
        ...
        logger.info(f"[OK] Loaded {len(df)} rows from CSV")
        
        # Normalize column names to lowercase for indicator/label pipeline
        ...
        logger.info(f"[OK] Collected data for {len(data)} symbols from CSV")
    else:
        logger.info("[SIGNAL] CSV not found, fetching from Binance API...")
