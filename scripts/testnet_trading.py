...
class TestnetTrader:
    ...
    def __init__(self, testnet_api_key: str, testnet_secret_key: str):
        ...
        with open("models/xgboost_features.pkl", "rb") as f:
            ...
            logger.info(f"[CHART] Loaded {len(self.feature_names)} futures features")
        ...
        logger.info("=" * 80)
        logger.info("[ROCKET] TESTNET TRADER INITIALIZED")
        logger.info("=" * 80)
    ...
    def setup_futures_account(self):
        ...
        try:
            ...
            logger.info("⚙️  Setting up futures account...")
    ...
    def get_tradeable_symbols(self, min_volume_usdt: float = 5_000_000) -> list:
        ...
        try:
            ...
            symbols.sort()
            logger.info(f"[CHART] Found {len(symbols)} tradeable symbols (${min_volume_usdt/1e6:.0f}M+ volume, TRADING status)")
    ...
    def open_position(self, signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if len(self.positions) >= self.max_positions:
                logger.info(f"  [SKIP]  Max positions ({self.max_positions}) reached")
                return False
            
            # Calculate position size
            quantity = self.calculate_position_size(symbol)
            if quantity <= 0:
                logger.warning(f"  [WARNING]  {symbol}: Invalid quantity {quantity}")
                return False
            ...
            logger.info(f"[OK] Position opened successfully!")
    ...
    def check_positions(self):
        ...
        try:
            ...
            for pos in active_positions:
                ...
                pnl_emoji = '[GREEN_CIRCLE]' if unrealized_pnl > 0 else '[RED_CIRCLE]'
    ...
    def import_existing_positions(self):
        ...
        try:
            ...
            for pos in active_positions:
                ...
                logger.info(f"  📥 Imported {action} position: {symbol} @ ${entry_price:,.4f} (qty: {quantity})")
            ...
            if not active_positions:
                ...
                logger.info(f"  [OK] Imported {len(active_positions)} position(s)")
    ...
    async def trading_loop(self):
        """Main trading loop."""
        logger.info("\n🔄 STARTING TESTNET TRADING LOOP...")
        
        # Setup account
        if not self.setup_futures_account():
            logger.error("❌ Failed to setup account, exiting")
            return
        ...
        while True:
            try:
                ...
                logger.info(f"\n{'='*80}")
                logger.info(f"[SEARCH] ITERATION #{iteration} - {current_time.strftime('%Y-%m-%d %H:%M:%S UTC')}")
                logger.info(f"{'='*80}")
                ...
                if len(self.positions) < self.max_positions:
                    logger.info(f"\n[TARGET] SCANNING FOR SIGNALS ({len(self.positions)}/{self.max_positions} positions)...")
                    ...
                    for symbol in symbols:
                        ...
                        if signal:
                            if signal['action'] == 'HOLD':
                                ...
                                logger.info(f"[WARNING] {symbol}: {signal['action']} but confidence {signal['confidence']:.2%} < {self.min_confidence:.0%}")
                        
                        if signal and signal['action'] != 'HOLD' and signal['confidence'] >= self.min_confidence:
                            logger.info(f"\n[ALERT] SIGNAL DETECTED: {symbol}")
                    ...
                    logger.info(f"\n⏸️  Max positions reached ({self.max_positions}), waiting for exits...")
        ...
        logger.info("\n" + "="*80)
        logger.info("🛑 TESTNET TRADING STOPPED")
        logger.info("="*80)
