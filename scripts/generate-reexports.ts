...
function makeStub(targetRelPath: string): string {
  // Import by base name so the legacy .js/.jsx file continues to resolve
  // the same way it did before (relative to the same directory).
...
function run(): void {
  ...
  // Consider both .ts and .tsx implementation files. For each TS implementation
  // file found, if a legacy .js or .jsx sibling exists, we'll replace the
  // legacy file with a small re-export stub pointing to the TS module.
  const all = walk(root).filter(f => f.endsWith('.ts') || f.endsWith('.tsx'));
  const planned: { legacy: string; target: string; relLegacy: string; relTarget: string }[] = [];
