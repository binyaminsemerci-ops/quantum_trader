...
if [ -z "$LEVERAGE_HITS" ]; then
    ...
    fail "Found hardcoded leverage=10" "$LEVERAGE_HITS"
fi
...
echo "------------------------------------------------------------"
MAX_LEV_DECISION=$(grep -RniE "MAX_LEVERAGE\s*=\s*10[^0-9]" \
...
if [ -z "$HARVEST_HARDCODE" ]; then
    ...
    if echo "$HARVEST_HARDCODE" | grep -q "risk_kernel_harvest.py"; then
        ...
        fail "Hardcoded harvest thresholds in services" "$HARVEST_HARDCODE"
    fi
fi
...
echo "----------------------------------------------------------------"
KILL_HARDCODE=$(grep -RniE "[Kk]_[Cc][Ll][Oo][Ss][Ee]_[Tt][Hh][Rr][Ee][Ss][Hh][Oo][Ll][Dd]\s*=\s*0\.[0-9]+" \
    microservices/portfolio_heat_gate/main.py \
    microservices/governor/main.py \
    --exclude-dir=__pycache__ \
    2>/dev/null | grep -v "config\|params.get" || true)

if [ -z "$KILL_HARDCODE" ]; then
    pass "No hardcoded kill score thresholds in decision logic"
else
    fail "Hardcoded kill score thresholds" "$KILL_HARDCODE"
fi
...
echo "----------------------------------------------------------------"
WEIGHT_HARDCODE=$(grep -RniE "score\s*=.*0\.[34].*\*|0\.[34].*\*.*score" \
...
if [ -z "$WEIGHT_HARDCODE" ]; then
    ...
    fail "Hardcoded symbol selection weights" "$WEIGHT_HARDCODE"
fi
...
echo "--------------------------------------------------------------------"
NOTIONAL_HARDCODE=$(grep -RniE "MAX_NOTIONAL_PER_TRADE_USDT\s*=\s*200" \
    microservices/governor/main.py \
    2>/dev/null || true)
...
if [ -z "$NOTIONAL_HARDCODE" ]; then
    ...
    if systemctl is-active --quiet quantum-governor 2>/dev/null; then
        fail "Hardcoded MAX_NOTIONAL in ACTIVE Governor service" "$NOTIONAL_HARDCODE"
    else
...
echo "------------------------------------------------------"
POLICY_IMPORTS=$(grep -l "from lib.policy_store import" \
    microservices/intent_bridge/main.py \
    microservices/harvest_proposal_publisher/main.py \
    2>/dev/null || true)

EXPECTED_COUNT=2
ACTUAL_COUNT=$(echo "$POLICY_IMPORTS" | grep -c "main.py" || echo 0)

if [ "$ACTUAL_COUNT" -eq "$EXPECTED_COUNT" ]; then
    pass "PolicyStore imported by $ACTUAL_COUNT/$EXPECTED_COUNT services"
else
    fail "PolicyStore imported by $ACTUAL_COUNT/$EXPECTED_COUNT services" "Missing in some services"
fi
...
echo "---------------------------------------------"
SKIP_LOGIC=$(grep -RniE "POLICY_MISSING|POLICY_STALE|SKIP.*policy" \
    microservices/intent_bridge/main.py \
    microservices/ai_engine/service.py \
    microservices/harvest_proposal_publisher/main.py \
    --exclude-dir=__pycache__ \
...
echo "------------------------------------------------------------------------"
SILENT_FALLBACKS=$(grep -RniE "leverage.*or\s+[0-9]+|size.*or\s+[0-9]+|threshold.*or\s+[0-9]" \
    microservices/ai_engine/service.py \
    microservices/intent_bridge/main.py \
    --exclude-dir=__pycache__ \
    2>/dev/null | grep -v "config.get\|params.get\|os.getenv" || true)

if [ -z "$SILENT_FALLBACKS" ]; then
    pass "No silent fallbacks in trading decision paths"
else
    fail "Found silent 'or default' fallbacks" "$SILENT_FALLBACKS"
fi
