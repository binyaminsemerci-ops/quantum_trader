...
def check_prerequisites():
    ...
    print("\n" + "="*70)
    print("[SEARCH] PRE-FLIGHT CHECKS FOR LIVE TRADING")
    print("="*70 + "\n")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data['mode'] == 'hybrid' and data['tft_loaded'] and data['xgb_loaded']:
                checks_passed.append("[OK] Hybrid Agent: ACTIVE (TFT + XGBoost)")
            else:
                checks_failed.append("❌ Hybrid Agent: Not fully loaded")
        else:
            checks_failed.append("❌ Hybrid Agent: Health check failed")
    except Exception as e:
        checks_failed.append(f"❌ Hybrid Agent: Cannot connect ({e})")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'healthy':
                checks_passed.append("[OK] Backend: HEALTHY")
            else:
                checks_failed.append("❌ Backend: Unhealthy")
        else:
            checks_failed.append("❌ Backend: Not responding")
    except Exception as e:
        checks_failed.append(f"❌ Backend: Cannot connect ({e})")
    
    # 3. Check .env file
    env_path = Path(".env")
    if env_path.exists():
        checks_passed.append("[OK] .env file: EXISTS")
        ...
        if has_api_key and has_secret:
            checks_passed.append("[OK] API Keys: CONFIGURED")
        else:
            checks_failed.append("❌ API Keys: NOT FOUND in .env")
    else:
    ...
    if tft_model.exists():
        ...
        checks_passed.append(f"[OK] TFT Model: EXISTS ({size_mb:.1f} MB)")
    else:
        checks_failed.append("❌ TFT Model: NOT FOUND")
    
    # 5. Check XGBoost model
    xgb_model = Path("ai_engine/models/xgb_model.pkl")
    if xgb_model.exists():
        checks_passed.append("[OK] XGBoost Model: EXISTS")
    else:
        checks_failed.append("❌ XGBoost Model: NOT FOUND")
...
def show_live_trading_warning():
    ...
    print("\n" + "[WARNING] "*35)
    print("                  [WARNING]  LIVE TRADING WARNING [WARNING]")
    print("[WARNING] "*35 + "\n")
    
    print("You are about to enable LIVE TRADING with REAL MONEY.")
    print("\nCurrent Configuration:")
    print("  • AI Model: HYBRID (TFT 60% + XGBoost 40%)")
    print("  • Leverage: 20x")
    print("  • Max per trade: $4000")
    ...
    print("  [WARNING]  High leverage = high risk")
    print("  [WARNING]  AI predictions are not guaranteed")
    print("\nSafeguards:")
    print("  [OK] Stop loss at -2%")
    print("  [OK] Max 4 positions ($16,000 total)")
    print("  [OK] Confidence threshold: 65%")
    print("  [OK] Event-driven mode (not continuous)")
    print("\n" + "="*70 + "\n")
...
def show_activation_instructions():
    ...
    print("\n" + "="*70)
    print("[MEMO] HOW TO ENABLE LIVE TRADING")
    print("="*70 + "\n")
    
    print("1. Edit docker-compose.yml:")
    print("   Line 30: QT_PAPER_TRADING=true → QT_PAPER_TRADING=false")
    print("   Line 31: STAGING_MODE=true → STAGING_MODE=false")
    print("")
...
def main():
    print("\n╔════════════════════════════════════════════════════════════════════╗")
    print("║           [ROCKET] LIVE TRADING ACTIVATION WIZARD [ROCKET]                    ║")
    print("╚════════════════════════════════════════════════════════════════════╝")
    ...
    if not confirm_live_trading():
        print("\n❌ Live trading activation CANCELLED by user")
        print("   System remains in paper trading mode.\n")
        return False
    
    print("\n[OK] USER CONFIRMED LIVE TRADING ACTIVATION\n")
