#!/usr/bin/env python3
"""
PatchTST P0.6 Retraining Script — FIX BUY BIAS + CONFIDENCE COLLAPSE
 
ROOT CAUSES (from P0.4):
- 60/40 WIN/LOSS imbalance → BUY bias (87%)
- No confidence regularization → stddev 0.003 (collapsed)
- Threshold alignment (prob=0.615 → BUY cutoff=0.6) → all BUY

FIXES (P0.6):
1. Stratified balanced sampling (50/50 WIN/LOSS)
2. Label smoothing (0.9/0.1 instead of 1/0)
3. Class weights (backup)
4. Sanity checks (action diversity + confidence spread HARD FAILS)
5. Confidence stats tracking
...
def prepare_features(df, balance_classes=True):
    ...
    P0.6 FIX: Balanced sampling to fix 87% BUY bias.
    """
    print(f"\n{'='*60}")
    print(f"PREPARING FEATURES (P0.6 BALANCED)")
    print(f"{'='*60}")
...
def evaluate_model(model, X_test, y_test):
    ...
    print(f"\n{'='*60}")
    print(f"TEST SET EVALUATION (P0.6)")
    print(f"{'='*60}")
...
def main():
    ...
    # Train model with P0.6 improvements
    ...
    # P0.6 SANITY CHECKS (HARD FAILS)
    sanity_pass = sanity_checks(eval_results)
    
    if not sanity_pass:
        print(f"\n{'='*70}")
        print(f"❌ P0.6 TRAINING FAILED SANITY CHECKS")
        print(f"{'='*70}")
        print(f"\n🔧 Adjust hyperparameters and retrain:")
        print(f"   1. Increase label_smoothing to 0.15")
        print(f"   2. Try higher dropout (0.2)")
        print(f"   3. Reduce learning rate to 0.0001")
        print(f"   4. Increase epochs to 30")
    ...
    # Save model
    model_filename = f"patchtst_v{timestamp}_p0_6.pth"
