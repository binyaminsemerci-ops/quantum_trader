"""
Train N-HiTS model for crypto trading
Multi-rate temporal pattern recognition
"""
import sys
import os
...
from ai_engine.nhits_simple import NHiTS  # Use simplified version
from ai_engine.nhits_model import NHiTSTrainer, save_model
...
def prepare_sequences(df: pd.DataFrame, sequence_length: int = 120):
    """Prepare sequences for N-HiTS training."""
...
def main():
    logger.info("=" * 60)
    logger.info("[ROCKET] N-HiTS TRAINING FOR CRYPTO TRADING")
    logger.info("=" * 60)
    ...
    # Create trainer
    trainer = NHiTSTrainer(model, device=device, learning_rate=0.001)
    ...
    for epoch in range(50):
        ...
        if val_metrics['loss'] < best_val_loss:
            ...
            model_path = Path("ai_engine/models/nhits_model.pth")
            ...
            metadata_path = Path("ai_engine/models/nhits_metadata.json")
    ...
    print("\n" + "="*60)
    print("[OK] N-HiTS TRAINING COMPLETE!")
    print("="*60)
    print(f"Best validation loss: {best_val_loss:.4f}")
    print(f"Model saved to: ai_engine/models/nhits_model.pth")
