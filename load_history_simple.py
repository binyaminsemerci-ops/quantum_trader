...
if not testnet_api_key or not testnet_secret:
    print("❌ Testnet API keys not found!")
...
print(f"[CHART] Fetching 2 hours of 1-minute data for {len(symbols)} symbols...")
...
for symbol in symbols:
    try:
        ...
        if klines:
            print(f"[OK] {symbol:12} - {len(klines)} candles from {start_time.strftime('%H:%M')} to {end_time.strftime('%H:%M')}")
        else:
...
print(f"\n[OK] Data fetched! Backend will now have access to this data.")
print(f"   N-HiTS and PatchTST models will start making predictions!")
print(f"\n💡 The models cache data in memory as backend scans symbols.")
