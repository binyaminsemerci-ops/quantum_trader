"""groq_client: PATCH-11 — Groq HTTP client with per-model rate throttle.

Single responsibility: POST /v1/chat/completions to Groq and return raw str.

All models share one API key. Each GroqModelClient instance owns its own
rate-throttle state. Instantiate one per model (primary, fallback, evaluator).
...
_log = logging.getLogger("exit_management_agent.groq_client")
...
class GroqModelClient:
    ...
    def __init__(
        ...
        _log.info(
            "[GroqClient] Initialised model=%s timeout=%.1fs throttle=%.1fs",
            self._model,
            self._timeout_sec,
    ...
    async def chat(self, system_prompt: str, user_content: str) -> str:
        ...
        Raises:
            ...
            ThrottleSkipError           — rate throttle fired (call came too soon)
        """
...
class ThrottleSkipError(Exception):
    """Raised by GroqModelClient.chat() when the per-model rate throttle fires."""
