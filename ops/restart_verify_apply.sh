...
redis-cli keys "quantum:position:*" 2>/dev/null | grep -v snapshot | grep -v cooldown | sort
echo ""
echo "=== ENTRY test: active positions count ==="
redis-cli keys "quantum:position:*" 2>/dev/null | grep -v snapshot | grep -v ledger | grep -v cooldown | wc -l | xargs echo "Active positions:"
