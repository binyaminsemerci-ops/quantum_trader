...
class ProductionTestRunner:
    ...
    def analyze_backtest_risk(self, backtest_result: Dict) -> Dict[str, Any]:
        ...
        if analysis["volatility"] > 0:
            analysis["risk_adjusted_return"] = analysis["total_return_pct"] / analysis["volatility"]
    ...
    def run_parallel_tests(
        ...
        logger.info(f"Running {len(test_combinations)} test combinations with {max_workers} workers")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_params = {
                ...
                for i, (symbols, limit, threshold, timeframe) in enumerate(test_combinations)
            }
        ...
        if "error" not in result:
            ...
            if "metrics" in result:
                ...
                self.performance_metrics["average_accuracy"] = (
                    (self.performance_metrics["average_accuracy"] * (self.performance_metrics["successful_tests"] - 1) + accuracy) /
                    self.performance_metrics["successful_tests"]
                )
            
            if "backtest" in result:
                pnl_pct = (result["backtest"].get("pnl", 0) / result["backtest"].get("starting_equity", 10000)) * 100
                self.performance_metrics["average_return"] = (
                    (self.performance_metrics["average_return"] * (self.performance_metrics["successful_tests"] - 1) + pnl_pct) /
                    self.performance_metrics["successful_tests"]
                )
        else:
    ...
    def run_configuration_test(self, config_name: str) -> Dict[str, Any]:
        ...
        final_result = {
            ...
            "test_summary": {
                ...
                "average_test_duration": (total_duration / len(test_results)) if test_results else 0
            },
        ...
        logger.info(f"Configuration {config_name} completed in {total_duration/60:.1f} minutes")
        logger.info(f"Results saved to: {result_file}")
        
        return final_result
    
    def analyze_test_results(self, results: List[Dict], config: TestConfiguration) -> Dict[str, Any]:
        ...
        for result in successful_results:
            ...
            if "backtest" in result:
                ...
                pnl_pct = (backtest.get("pnl", 0) / backtest.get("starting_equity", 10000)) * 100
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "test_session_duration": (datetime.now() - self.performance_metrics["test_start_time"]).total_seconds() / 60,
...
def main():
    ...
    parser.add_argument("--config", choices=list(PRODUCTION_TEST_CONFIGS.keys()), 
                       help="Run specific test configuration")
    parser.add_argument("--all-configs", action="store_true",
                       help="Run all test configurations")
    parser.add_argument("--output-dir", type=str, default="production_test_results",
    ...
    if args.config:
        ...
        print(f"⏱️  Duration: {result['test_summary']['total_duration_minutes']:.1f} minutes")
        
        if "performance_analysis" in result and "return_stats" in result["performance_analysis"]:
    ...
    print(f"🎉 Production testing complete!")
