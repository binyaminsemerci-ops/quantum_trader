...
def get_ai_trading_status():
    ...
    with _state_lock:
        return {
            ...
            "continuous_learning_status": "Active" if _ai_state["learning_active"] else "Inactive",
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
...
async def get_recent_signals(limit: int = 5):
    """Get recent signals"""
    return [
        {"symbol": "BTCUSDT", "side": "buy", "confidence": 0.9, "timestamp": datetime.now(timezone.utc).isoformat()},
        {"symbol": "ETHUSDT", "side": "sell", "confidence": 0.8, "timestamp": datetime.now(timezone.utc).isoformat()},
    ]
