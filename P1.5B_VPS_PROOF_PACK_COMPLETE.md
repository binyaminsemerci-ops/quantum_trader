# P1.5B — VPS DEPLOYMENT + PROOF PACK ✅
...
1. ❌ **ModuleNotFoundError**: Python couldn't import `ai_engine.risk_kernel_stops`
...
2. ❌ **Service stuck in "activating"**: Restart loop with 10s delay
...
- ✅ Only calc-only modules: risk_kernel_stops (P1)

**Status**: ✅ PASS

---

### ✅ PROOF 3: P0.5 MarketState Data in Redis
...
**Status**: ✅ PASS — P0.5 MarketState active and publishing

---

### ✅ PROOF 4: P1.5 Risk Proposals Published to Redis
...
**Analysis**:
- ✅ All 3 symbols have proposals
- ✅ Proposals contain: proposed_sl, proposed_tp, stop_dist_pct, tp_dist_pct, trail_gap_pct
- ✅ MarketState data embedded: ts, sigma, p_trend, p_mr, p_chop
- ✅ Position context: side, age, current_price, entry_price
- ✅ Reason codes present: "trail_active,sl_tightening,regime_chop"

**Status**: ✅ PASS — P1.5 Risk Proposal Publisher working correctly

---

### ✅ PROOF 5: --once Mode (Single Cycle)
...
**Analysis**:
- ✅ Script executed exactly one cycle
- ✅ Published proposals for all 3 symbols
- ✅ Exited cleanly after cycle (no exception)
- ✅ --once flag respected
...
### ✅ PROOF 6: Rate Limiting (10s Interval)
...
**Analysis**:
- ✅ Cycles occur every ~10 seconds (config: PUBLISH_INTERVAL_SEC=10)
- ✅ No excessive rapid publishing
...
|------|-------------|--------|
| 1 | systemd service is active | ✅ PASS |
| 2 | Calc-only import hygiene | ✅ PASS (no trading imports) |
| 3 | P0.5 MarketState in Redis | ✅ PASS (all 3 symbols) |
| 4 | P1.5 proposals in Redis | ✅ PASS (proposals published) |
| 5 | --once mode works | ✅ PASS (single cycle, clean exit) |
| 6 | Rate limiting (10s) | ✅ PASS (10s intervals observed) |

---

## NEXT STEPS

P1.5 deployment is **LOCKED AND VERIFIED** ✅

**Ready for P2: Trade Intent Generator**
- P1.5 provides proposals → P2 will consume and generate TradeIntents
...
✅ **P0 MarketState LOCKED SPEC v1.0** — VPS proof pack passed (14/14)  
✅ **P1 Risk Kernel LOCKED SPEC v1.0** — VPS proof pack passed (13/13)  
✅ **P1.5 Risk Proposal Publisher LOCKED** — VPS proof pack passed (6/6)

---

**🎯 P1.5B VPS DEPLOY + PROOF PACK: COMPLETE ✅**
