...
export default function Backtest() {
  ...
  const runBacktest = async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/backtest?symbol=${symbol}&days=30`
      );
  ...
  return (
    <div className="p-6 space-y-6 dark:bg-gray-900 dark:text-white min-h-screen">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white dark:bg-gray-800 p-4 shadow rounded-lg">
              ...
              <p className="text-xl font-bold">
                {result.mode === "trades" ? "Based on Trades" : "Based on Candles"}
              </p>
            </div>
            <div className="bg-white dark:bg-gray-800 p-4 shadow rounded-lg">
              <h3>PnL</h3>
      ...
      {result?.error && (
        <div className="p-4 bg-red-600 text-white rounded">{result.error}</div>
      )}
    </div>
  );
}
