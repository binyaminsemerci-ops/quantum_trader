...
print("\n" + "=" * 80)
print("[SEARCH] POSITION MONITOR CONFIGURATION")
print("=" * 80)
...
print(f"   → Close {float(partial_tp)*100:.0f}% of position (lock profit)")
print(f"   → Keep {(1-float(partial_tp))*100:.0f}% running with trailing stop")
print()
print(f"2. Trailing stop activates at +{float(trail_pct)*100:.1f}% profit:")
print(f"   → Follows price up, protecting gains")
print(f"   → Only for the remaining {(1-float(partial_tp))*100:.0f}% position")
print()
print(f"3. Stop Loss backup at -{float(sl_pct)*100:.1f}%:")
print(f"   → Closes entire position if loss reaches this level")
...
print(f"Status:         ACTIVE [OK] (running in backend)")

print("\n💡 Current APTUSDT Example:")
print("-" * 80)
...
print(f"     → Closes 614.5 APT (50%)")
print(f"")
print(f"  2. TRAILING_STOP for 614.4 APT (50%)")
print(f"     → Activates at +1.5% profit")
print(f"     → Follows price dynamically")
print(f"")
print(f"  3. STOP_MARKET @ $2.818 (-2.0%)")
print(f"     → Emergency backup for full position")
