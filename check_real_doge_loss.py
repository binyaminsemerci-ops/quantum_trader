...
if not doge_pos:
    print("❌ Ingen åpen DOGEUSDT posisjon i database")
    exit(1)

print(f"\n[SEARCH] DOGEUSDT POSISJON FRA DATABASE:")
print(f"{'='*60}")
...
if doge_pos.side == 'SHORT':
    ...
    print(f"\n[WARNING]  Stop Loss burde være: ${sl_price:.6f}")
    print(f"Avstand til SL: {((sl_price - mark_price) / mark_price * 100):.2f}%")
    
    if abs(pnl_pct) > 2:
        print(f"\n[ALERT] KRITISK: Tap {abs(pnl_pct):.2f}% > 2% stop loss!")
