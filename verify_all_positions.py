...
print("\n" + "=" * 100)
print("[SEARCH] COMPLETE POSITION & ORDER VERIFICATION")
print("=" * 100)
...
print(f"\n[CHART] Found {len(active_positions)} active position(s)")
print("=" * 100)
...
for idx, pos in enumerate(active_positions, 1):
    ...
    print(f"\n{idx}. {'[RED_CIRCLE] SHORT' if amt < 0 else '[GREEN_CIRCLE] LONG'} {symbol}")
    print("-" * 100)
    ...
    print(f"\n   [CLIPBOARD] Expected Protection Levels:")
    ...
    if not orders:
        print(f"\n   ❌ NO ORDERS FOUND! Position is UNPROTECTED!")
        continue
    
    print(f"\n   [OK] Active Orders ({len(orders)}):")
    ...
    for order in orders:
        ...
        if order_type == 'TAKE_PROFIT_MARKET':
            ...
            if order.get('closePosition'):
                sl_ok = True
                notes.append("closePosition=True ✓")
            else:
    ...
    if issues:
        ...
        print(f"      [OK] ALL ORDERS CORRECTLY SET!")
