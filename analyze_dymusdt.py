...
print("\n" + "=" * 80)
print("[CHART] DYMUSDT POSITION ANALYSIS")
print("=" * 80)
...
print(f"\n[SHIELD] Our System Orders (from Position Monitor):")
print(f"   TP @ ${tp_price:.6f} (-3.0% = +3% profit)")
print(f"      → Closes 50% (25,294 DYM)")
print(f"   ")
print(f"   Trailing Stop for remaining 50%")
print(f"      → Activates at ${trail_trigger:.6f} (-1.5% = +1.5% profit)")
print(f"   ")
print(f"   SL @ ${sl_price:.6f} (+2.0% = -2% loss)")
print(f"      → Emergency backup")

# Current status
current_move = ((entry_price - mark_price) / entry_price) * 100
print(f"\n[CHART_UP] Current Status:")
...
if current_move >= 1.5:
    print(f"   [TARGET] TRAILING STOP IS ACTIVE! Following price down")
elif current_move >= 0:
    print(f"   [OK] In profit, approaching trailing activation")
else:
    print(f"   [WARNING] In loss")

print(f"\n💡 What happens next:")
print(f"-" * 80)
if mark_price <= tp_price:
    print(f"[OK] TP TARGET REACHED!")
    print(f"   → 50% position (25,294 DYM) will close automatically")
    print(f"   → Locking in ~{pnl/2:.2f} BNFCR profit")
    print(f"   → Remaining 50% continues with trailing stop")
elif current_move >= 1.5:
    print(f"[CHART_UP] Trailing stop is ACTIVE and protecting your gains!")
    print(f"   → Follows price down (in your favor)")
    print(f"   → Will close if price reverses by {1.5}%")
else:
    print(f"[TARGET] Position is working towards TP at ${tp_price:.6f}")
    print(f"   → Need price to drop {((mark_price - tp_price) / mark_price * 100):.2f}% more")
