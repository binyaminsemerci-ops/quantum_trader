...
```bash
# Live PnL events
docker exec quantum_redis redis-cli XREAD STREAMS quantum:stream:exitbrain.pnl 0

# RL rewards per symbol
docker exec quantum_redis redis-cli KEYS "quantum:rl:reward:*"
docker exec quantum_redis redis-cli GET quantum:rl:reward:BTCUSDT
```

### 3. Container Logs
```bash
# Exit Brain Executor
docker logs quantum_exit_brain_executor --follow

# Auto Executor
docker logs quantum_auto_executor --follow

# Binance PnL Tracker
docker logs quantum_binance_pnl_tracker --follow
```
...
docker exec quantum_redis redis-cli KEYS "*exit_plan*"

# Monitor PnL stream
docker exec quantum_redis redis-cli XREAD COUNT 10 STREAMS quantum:stream:exitbrain.pnl 0
```
...
docker logs quantum_exit_brain_executor | grep "ADAPTIVE_LEVELS"

# Monitor harvest executions
docker logs quantum_exit_brain_executor | grep "EXIT_TP_ORDER"

# View harvest scheme per position
docker exec quantum_redis redis-cli HGETALL "position:state:BTCUSDT:LONG"
```
