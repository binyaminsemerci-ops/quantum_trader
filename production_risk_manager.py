...
class RiskManager:
    ...
    def calculate_position_size(
        ...
        max_risk_amount = self.portfolio.available_equity * (self.params.max_position_size_pct / 100)
        ...
        max_equity_position = self.portfolio.available_equity * (self.params.max_position_size_pct / 100) / entry_price
        ...
        risk_info = {
            ...
            "portfolio_risk_pct": (position_size * entry_price / self.portfolio.total_equity) * 100,
    ...
    def validate_trade_signal(
        ...
        current_drawdown = ((self.equity_peak - self.portfolio.total_equity) / self.equity_peak) * 100
    ...
    def add_position(
        ...
        try:
            ...
            if stop_loss is None or take_profit is None:
                calc_stop, calc_tp = self.calculate_stop_loss_take_profit(entry_price, side)
            ...
            # Log the position
            logger.info(f"Added position: {symbol} {side} {quantity}@{entry_price} SL:{stop_loss} TP:{take_profit}")
    ...
    def update_portfolio(self, current_prices: Dict[str, float]):
        ...
        self.portfolio.total_equity = self.portfolio.available_equity + total_unrealized_pnl
    ...
    def close_position(self, symbol: str, exit_price: float, reason: str = "manual"):
        ...
        if position_to_close.side == 'long':
            pnl = (exit_price - position_to_close.entry_price) * position_to_close.quantity
        else:
            pnl = (position_to_close.entry_price - exit_price) * position_to_close.quantity
        
        # Update portfolio
        self.portfolio.available_equity += (position_to_close.entry_price * position_to_close.quantity) + pnl
...
def test_risk_manager():
    ...
    position_size, risk_info = rm.calculate_position_size(entry_price, stop_loss, signal_confidence=0.8)
    print(f"Position size: {position_size:.6f}")
    print(f"Risk info: {risk_info}")
    
    # Test trade validation
    print("\n✅ Testing Trade Validation:")
    is_valid, validation_info = rm.validate_trade_signal("BTCUSDT", "long", entry_price, signal_strength=0.8)
