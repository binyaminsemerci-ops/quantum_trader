...
**Threat Model:**
- ❌ Attacker with Redis credentials
- ❌ Rogue process publishing to `quantum:stream:reconcile.close`
- ❌ Replay attacks with valid plans
- ✅ HMAC prevents all unauthorized trades

### Security Flow

```
P3.4 Reconcile Engine:
├─ Generates plan with qty, symbol, timestamp, signature
├─ Computes HMAC-SHA256 over plan_id|symbol|qty|ts|signature
├─ Adds hmac field to Redis stream
└─ Plan published with cryptographic proof

Apply Layer:
├─ Reads plan from Redis stream
├─ Recomputes expected HMAC using shared secret
├─ Compares with hmac.compare_digest (timing-safe)
├─ INVARIANT 0: HMAC must match
...
4. If mismatch → Hard fail with metric `guardrail_fail{rule="hmac"}`
...
5. If failures persist > 30s → rollback to old secret
...
# Expected Result: ❌ REJECTED (Dedupe key or plan_id collision)
...
|--------|-------------|-----------|
| **Redis Access** | ❌ Can inject RECONCILE_CLOSE | ✅ Cannot forge valid HMAC |
| **Rogue Process** | ❌ Can publish to stream | ✅ No secret = rejected |
| **Replay Attack** | ⚠️ Dedupe only (86400s window) | ✅ Dedupe + HMAC both enforce |
| **Insider Threat** | ❌ Any engineer with Redis creds | ✅ Need both Redis + secret |
| **Config Error** | ❌ Accidental plan injection | ✅ Fails safe (no HMAC = reject) |
...
**HMAC Implementation:** ✅ **COMPLETE - PRODUCTION-READY**

**Security Level:** 🔒 **CRYPTOGRAPHIC-GRADE** (HMAC-SHA256)
...
**Threat Status:** ✅ All known attack vectors mitigated
