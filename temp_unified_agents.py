...
if TORCH_AVAILABLE:
    class NHiTSModel(nn.Module):
        """N-HiTS matching actual trained model (stacks with BatchNorm + fc)"""
        def __init__(self, input_size=18, hidden_size=128, num_stacks=4, num_classes=3, **kwargs):
            super().__init__()
            self.input_size = input_size
            self.hidden_size = hidden_size
            
            # Stacks with Dropout (matches v5 trained model: 0=Linear, 1=ReLU, 2=Dropout, 3=Linear)
            self.stacks = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(input_size if i == 0 else hidden_size, hidden_size),  # 0
                    nn.ReLU(),                                                       # 1
            ...
            self.fc = nn.Sequential(
                nn.Linear(hidden_size, hidden_size // 2),  # 0: 128->64
                nn.ReLU(),                                  # 1
                nn.Dropout(0.1),                            # 2
                nn.Linear(hidden_size // 2, num_classes)   # 3: 64->3
            )
    ...
    class PatchTSTModel(nn.Module):
        """PatchTST matching actual trained model (input_proj + transformer + fc)"""
        def __init__(self, input_dim=18, d_model=64, n_heads=4, n_layers=2, dropout=0.1, num_classes=3, **kwargs):
            super().__init__()
            # Input projection (NOT embedding)
            self.embedding = nn.Linear(input_dim, d_model)
            
            # Transformer with dim_feedforward=256 (d_model * 4)
            encoder_layer = nn.TransformerEncoderLayer(
                d_model=d_model, 
                nhead=n_heads, 
                dim_feedforward=d_model * 16,  # 2048 for d_model=128 (v5 matches)
                dropout=dropout, 
                batch_first=True
            )
            self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)
            
            # fc: d_model -> d_model//2 -> num_classes
            self.fc = nn.Sequential(
                nn.Linear(d_model, d_model // 2),      # 0: 64->32
                nn.ReLU(),                              # 1
                nn.Dropout(dropout),                    # 2
                nn.Linear(d_model // 2, num_classes)   # 3: 32->3
            )
    ...
    class GatedResidualNetwork(nn.Module):
        ...
        def forward(self, x):
            ...
            h = self.dropout(h)
            h = self.fc2(h)
            # Gate uses concatenation of input and hidden
    ...
    class VariableSelectionNetwork(nn.Module):
        ...
        def forward(self, x):
            ...
            weights = self.gating(x)  # [batch, features]
            weighted = x * weights
    ...
    class TemporalFusionBlock(nn.Module):
        ...
        def forward(self, x, hidden=None):
            # Static context (use last timestep)
            if x.dim() == 3:
    ...
    class TFTModel(nn.Module):
        def __init__(self, input_size=14, hidden_size=128, num_heads=8, num_layers=3, 
                     num_classes=3, dropout=0.1, sequence_length=120):
            super().__init__()
            self.input_size = input_size
            self.hidden_size = hidden_size
            ...
            # Quantile head for confidence
            self.quantile_head = nn.Sequential(
                nn.Linear(hidden_size, hidden_size // 2),
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(hidden_size // 2, 3)  # lower, median, upper quantiles
            )
            
            self.layer_norm = nn.LayerNorm(hidden_size)

        def forward(self, x):
            # x: [batch, features] for single step or [batch, seq, features] for sequence
            if x.dim() == 2:
                x = x.unsqueeze(1)  # [batch, 1, features]
            ...
            x = torch.stack(vsn_out, dim=1)  # [batch, seq, hidden]
            
            # Encoder LSTM
            enc_out, _ = self.encoder_lstm(x)  # [batch, seq, hidden*2]
            enc_out = self.encoder_projection(enc_out)  # [batch, seq, hidden]
            
            # Self-attention
            attn_out, _ = self.attention(enc_out, enc_out, enc_out)
            attn_out = self.grn1(attn_out[:, -1, :])  # Use last timestep
...
class NHiTSAgent(BaseAgent):
    ...
    def _load_pytorch_model(self, state_dict, meta_path):
        ...
        try:
            ...
            block_keys = [k for k in actual_state_dict.keys() if k.startswith("stacks.")]
            if block_keys:
...
class TFTAgent(BaseAgent):
    def __init__(self):
        super().__init__("TFT-Agent", "tft_model")
        self.pytorch_model = None
        self.feature_mean = None
        self.feature_std = None
        self._load()

    def _load_pytorch_model(self, checkpoint, meta_path):
        if not TORCH_AVAILABLE or TFTModel is None:
            self.logger.e("PyTorch or TFTModel not available")
            return None
        try:
            ...
            self.logger.i(f"Reconstructing TFT: input={input_size}, hidden={hidden_size}")
            ...
            param_count = sum(p.numel() for p in model.parameters())
            self.logger.i(f"✅ TFT loaded ({param_count:,} parameters)")
            return model
        except Exception as e:
            self.logger.e(f"Failed to reconstruct TFT: {e}")
            return None

    def _load(self, model_path=None, scaler_path=None):
        model_path = model_path or self._find_latest()
        if not model_path:
            self.logger.w("No TFT model found")
            self.ready = False
            return
        try:
            checkpoint = torch.load(model_path, map_location="cpu", weights_only=False)
            self.model = checkpoint
            self.pytorch_model = self._load_pytorch_model(checkpoint, None)
            if self.pytorch_model:
                config = checkpoint.get("model_config", {})
                self.features = [f"f{i}" for i in range(config.get("input_size", 14))]
                self.version = "v1"
                self.ready = True
                self.logger.i(f"✅ Loaded TFT model (features={len(self.features)})")
            else:
                self.ready = False
        except Exception as e:
            self.logger.e(f"Failed to load TFT: {e}")
            self.ready = False

    def predict(self, sym, feat):
        if self.pytorch_model is None or not TORCH_AVAILABLE:
            return {"symbol": sym, "action": "HOLD", "confidence": 0.5, "confidence_std": 0.1, "version": "dummy"}
        try:
            feature_keys = ["Close", "Volume", "EMA_10", "EMA_50", "RSI", "MACD", "MACD_signal",
                          "BB_upper", "BB_middle", "BB_lower", "ATR", "volume_sma_20",
                          "price_change_pct", "high_low_range"]
            feature_vec = [float(feat.get(k, feat.get(k.lower(), 0.0)) or 0.0) for k in feature_keys]
            X = np.array([feature_vec], dtype=np.float32)
            if self.feature_mean is not None and self.feature_std is not None:
                mean = np.array(self.feature_mean, dtype=np.float32)
                std = np.array(self.feature_std, dtype=np.float32)
            ...
            actions = ["SELL", "HOLD", "BUY"]
            act = actions[class_pred]
            self.logger.i(f"{sym} → {act} (conf={confidence:.3f})")
            return {"symbol": sym, "action": act, "confidence": confidence, "confidence_std": 0.1, "version": self.version}
        except Exception as e:
            self.logger.e(f"TFT prediction failed: {e}")
            return {"symbol": sym, "action": "HOLD", "confidence": 0.5, "confidence_std": 0.1, "version": "error"}
